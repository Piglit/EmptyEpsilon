...
def test_template():
	...
	core.activity(crew, "did nothing", "nothing")
	assert getCrew(serverName+"1").getScenarios() == [models.scenario.getScenario("20_training1")]
	assert getCrew(serverName+"1").getShips() == ["Phobos M3P"]
	assert getCrew(serverName+"1").getBriefing() == {"Instructions": "Hello Crew!"}
	assert getCrew(serverName).getScenarios() == []
	assert getCrew(serverName).getShips() == []
	assert getCrew(serverName).getBriefing() == {}
	...
	core.activity(crew, "did nothing", "nothing")
	assert getCrew(serverName+"2").getBriefing() == {"Instructions": f"Hello Crew der {crewName}2!"}
	setCrewTemplate([], [], "")
	removeCrew(serverName)
...
def test_briefing():
	...
	assert crew.getBriefing() == {"Instructions": "Test"}
	crew.setBriefing({"Keks": "Test"})
	assert crew.getBriefing() == {"Keks": "Test"}
	crew.setBriefing({"Keks": "Test", "K2": "T2"})
	assert crew.getBriefing() == {"Keks": "Test", "K2": "T2"}
	removeCrew(serverName)
