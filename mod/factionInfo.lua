...
imp = FactionInfo():setName("Imperial"):setLocaleName(_("Imperial"))
imp:setGMColor(0, 255, 0)
imp:setDescription(_([[Remnants of the Imperial Forces]]))

alliance = FactionInfo():setName("New Republic"):setLocaleName(_("New Republic"))
alliance:setGMColor(255, 0, 0)
alliance:setDescription(_([[The forces of the New Republic]]))
alliance:setEnemy(imp)

cd = FactionInfo():setName("Crimson Dawn"):setLocaleName(_("Crimson Dawn"))
cd:setGMColor(255, 255, 0)
cd:setDescription(_([[Ships of the Crimson Dawn syndicate]]))
cd:setEnemy(imp)
cd:setEnemy(alliance)
...
for i=1,4 do
	...
	player_factions:setFriendly(player_factions[i])
end

tantal = FactionInfo():setName("Endor"):setLocaleName(_("Endor"))
tantal:setGMColor(0, 196, 196)
tantal:setDescription(_([[Endor flight control and navigation infrastructure.]]))

enemies = FactionInfo():setName("Raider"):setLocaleName(_("Raider"))
enemies:setGMColor(255, 0, 255)
enemies:setDescription(_([[Raiders that may attack (more or less) innocent transportation vehicles.]]))
player_factions:setEnemy(enemies)

natural = FactionInfo():setName("Environment"):setLocaleName(_("Environment"))
natural:setGMColor(128, 128, 128)
natural:setDescription(_([[Debris flying around.]]))
player_factions:setEnemy(natural)

sky = FactionInfo():setName("Sky Patrol"):setLocaleName(_("Sky Patrol"))
sky:setGMColor(0, 255, 255)
sky:setDescription(_([[Endor Sky Patrol - they enforce the law in orbit.]]))
...
reds:setEnemy(cd)
reds:setEnemy(natural)
reds:setEnemy(tantal)
reds:setEnemy(sky)
player_factions:setEnemy(reds)
-- neutral to raiders
