--[[ Comms functions
Here is a set of functions, that can be called from comms scripts.
To call them, give comms_source/player and comms_target/station as arguments.
...
-- calculate panic range, store it in comms_data and return it.
function comms_functions.getPanicRange(target)
	if target.comms_data.panic_range ~= nil then
		return target.comms_data.panic_range
	end
	local panic_range = 5000
    local temp_type = target:getTypeName()
    local range_divisor = {
        ["Small Station"]    = 2,
        ["Medium Station"]    = 3,
        ["Large Station"]    = 4,
    ...
    if temp_type == nil or range_divisor[temp_type] == nil then
        print("template name nil for:",target:getCallSign(),"defaulting panic range to 5000")
    else
        panic_range = target:getShortRangeRadarRange()/range_divisor[temp_type]    
    end
    if panic_range == nil then
        print("calculating panic range failed. Defaulting to 5000")
        panic_range = 5000
    end
	target.comms_data.panic_range = panic_range
	return panic_range
end


function comms_functions.addStationToDatabase(source, station)
    ...
    if station:isFriendly(source) then
    ...
    elseif not station:isEnemy(source) then
        local neutral_key = "Neutral"
        local neutral_db = queryScienceDatabase(stations_key,neutral_key)
        if neutral_db == nil then
...
function comms_functions.selectMessageByFriendlyness(messages, friendlyness)
	local friendlyness = friendlyness or irandom(1,100)	--eg 65
	-- #messages: first good, last bad
	local interval = 100 / #messages		-- eg 10
	local index = math.min(#messages, math.max(1, math.ceil(friendlyness / interval)))	-- 6(.5), clamp between [1,#messages] (1,100)
	return messages[index]
end

function comms_functions.stationStatusReport(source, target)
	msg = string.format(_("situationReport-comms","Hull:%s    "),math.floor(target:getHull() / target:getHullMax() * 100))
	local shields = target:getShieldCount()
	if shields == 1 then
		msg = string.format(_("situationReport-comms","%s  Shield:%s"),msg,math.floor(target:getShieldLevel(0) / target:getShieldMax(0) * 100))
	else
		for n=0,shields-1 do
			msg = string.format(_("situationReport-comms","%s  Shield %s:%s"),msg,n,math.floor(target:getShieldLevel(n) / target:getShieldMax(n) * 100))
		end
	end
	local improvements = {}
	...
	for i,system in ipairs(system_list) do
		if target.comms_data.system_repair[system].avail then
	...
	for i,system in ipairs(secondary_system_repair_desc) do
		if target.comms_data[system.name] then
	...
	if target.comms_data.jump_overcharge then
		overcharge_service = "jump drive"
	end
	if target.comms_data.shield_overcharge then
	...
	return msg
        ...
        --if #improvements > 0 and (target.comms_data.friendlyness > 33 or source:isDocked(target)) then
...
function populateDefenseFleet(station)
	...
	local station_type = target:getTypeName()
	local size_matters = {
		["Small Station"] = -5,
		["Medium Station"] = 0,
		["Large Station"] = 5,
	...
	for i,data in ipairs(station.comms_data.defense_fleet_chances) do
		...
		if random(1,100) < chance then
			target.comms_data.idle_defense_fleet["DF"..tostring(i+1)] = ship
		else
			break
		end
	end
...
function stationDefenseFleet(calling_function)
    ...
    if target.comms_data.idle_defense_fleet == nil then
		populateDefenseFleet(target)
    end
    local defense_fleet_count = 0
    for name, template in pairs(target.comms_data.idle_defense_fleet) do
    ...
    if defense_fleet_count > 0 then
        local fleet_prompts = {
            ...
            string.format("Launch %s defenders (%s rep)",target:getCallSign(),getServiceCost("activatedefensefleet")),
            string.format("Enable %s defenders (%s rep)",target:getCallSign(),getServiceCost("activatedefensefleet")),
        }
        addCommsReply(tableSelectRandom(fleet_prompts),function()
            if source:takeReputationPoints(getServiceCost("activatedefensefleet")) then
                for name, template in pairs(target.comms_data.idle_defense_fleet) do
                    local script = Script()
                    local position_x, position_y = target:getPosition()
                    local station_name = target:getCallSign()
                    ...
                    script:setVariable("faction_id",source:getFactionId())
                    script:run("border_defend_station.lua")
                    target.comms_data.idle_defense_fleet[name] = nil
                end
                local launched_responses = {
                    "Defense fleet activated",
                    "Defenders launched",
                    string.format("%s defense fleet activated",target:getCallSign()),
                    string.format("Station %s defenders engaged",target:getCallSign()),
                    string.format("%s defenders enabled",target:getCallSign()),
                }
                ...
                local insufficient_rep_responses = {
                    ...
                    string.format("You need more than %i reputation",math.floor(source:getReputationPoints())),
...
function undockedLightPanic(source, target, oMsg)
	...
	local panic_range = target.comms_data.panic_range
    if target:areEnemiesInRange(panic_range*1.5) then
        if target.comms_data.friendlyness > 20 then
            oMsg = string.format(_("station-comms","%s\n%s"),oMsg,tableSelectRandom(no_relay_panic_responses))
            setCommsMessage(oMsg)
        end
    elseif target:areEnemiesInRange(panic_range*2) then
        if target.comms_data.friendlyness > 75 then
            ...
            if target.comms_data.friendlyness > 40 then
                oMsg = string.format(_("station-comms","%s\n%s"),oMsg,tableSelectRandom(no_relay_panic_responses))
                setCommsMessage(oMsg)
            end
        end
    elseif target:areEnemiesInRange(panic_range*2.5) then
        if target.comms_data.friendlyness > 33 then
            if target.comms_data.friendlyness > 75 then
                local distracted_units_responses = {
                    ...
                    string.format("A busy day here at %s: Enemies are %s units away and my boss is reviewing emergency procedures. I'm a bit distracted.",target:getCallSign(),math.floor(panic_range*2.5/1000)),
                ...
                setCommsMessage(oMsg)
            elseif target.comms_data.friendlyness > 50 then
                local distracted_responses = {
                    "Please forgive us if we seem distracted. Our sensors detect enemies nearby.",
                    string.format("Enemies are close to %s. We might get busy. Business?",target:getCallSign()),
                    ...
                    string.format("%s is likely going to be attacked soon. Everyone is running around getting ready, distracting me.",target:getCallSign()),
                }
...
--	oMsg = string.format("(DEBUG: friendlyness: %f)\n%s", target.comms_data.friendlyness, oMsg) -- TODO remove (debug)
--	table.dump(target.comms_data)-- TODO remove (debug)
    setCommsMessage(oMsg)
	if interactive then
...
function catalogImprovements(msg)
    ...
    if target:getRestocksScanProbes() then
        ...
        if target.probe_fail_reason == nil then
            ...
            target.probe_fail_reason = reason_list[math.random(1,#reason_list)]
        end
        msg = string.format("%s\n%s",msg,target.probe_fail_reason)
        table.insert(improvements,"restock_probes")
    end
    if target:getRepairDocked() then
        msg = string.format(_("situationReport-comms","%s\nRepair ship hull: nominal."),msg)
    else
        if target.repair_fail_reason == nil then
            reason_list = {
            ...
            target.repair_fail_reason = reason_list[math.random(1,#reason_list)]
        end
        msg = string.format("%s\n%s",msg,target.repair_fail_reason)
        table.insert(improvements,"hull")
    end
    if target:getSharesEnergyWithDocked() then
        msg = string.format(_("situationReport-comms","%s\nRecharge ship energy stores: nominal."),msg)
    else
        if target.energy_fail_reason == nil then
            reason_list = {
            ...
            target.energy_fail_reason = reason_list[math.random(1,#reason_list)]
        end
        msg = string.format("%s\n%s",msg,target.energy_fail_reason)
    ...
    for i,m_type in ipairs(missile_types_desc) do
        if target.comms_data.weapon_available == nil then
            print("weapon available for station",target:getCallSign(),"is nil")
        else
            if target.comms_data.weapon_available[m_type.name] then    --problem child weapon_available is nil
...
function improveStationService(improvements)
    ...
    addCommsReply(_("situationReport-comms","Improve station services"),function()
        local improve_what_service_prompts = {
            ...
            string.format("Which %s service can %s help improve?",target:getCallSign(),source:getCallSign()),
        }
        if #improvements == 1 then
            improve_what_service_prompts = {
                ...
                string.format("%s can help %s by improving this service:",source:getCallSign(),target:getCallSign()),
        ...
        for i,improvement in ipairs(improvements) do
            if improvement_prompt[improvement] == nil then
                ...
                addCommsReply(tableSelectRandom(improvement_prompt[improvement]),function()    
                    local needed_good = target.mission_goods[improvement]
                    ...
                    if source.goods ~= nil then
                        if source.goods[needed_good] ~= nil and source.goods[needed_good] > 0 and source:isDocked(target) then
                            addCommsReply(string.format(_("situationReport-comms","Provide %s to station %s"),needed_good,target:getCallSign()),function()
                                if source:isDocked(target) then
                                    source.goods[needed_good] = source.goods[needed_good] - 1
                                    source.cargo = source.cargo + 1
                                    ...
                                    if improvement == "energy" then
                                        if source.instant_energy == nil then	-- TODO make it work!
                                            source.instant_energy = {}
                                        end
                                        table.insert(source.instant_energy,target)
                                        target:setSharesEnergyWithDocked(true)
                                        improvement_msg = _("situationReport-comms","We can recharge again! Come back any time to have your batteries recharged.")
                                        target.comms_data.friendlyness = math.min(target.comms_data.friendlyness + random(friendliness_bonus_lo,friendliness_bonus_hi),100)
                                    elseif improvement == "hull" then
                                        if source.instant_hull == nil then
                                            source.instant_hull = {}
                                        end
                                        table.insert(source.instant_hull,target)
                                        target:setRepairDocked(true)
                                    ...
                                    elseif improvement == "restock_probes" then
                                        if source.instant_probes == nil then
                                            source.instant_probes = {}
                                        end
                                        table.insert(source.instant_probes,target)
                                        target:setRestocksScanProbes(true)
                                    ...
                                    elseif improvement == "Nuke" then
                                        if source.nuke_discount == nil then
                                            source.nuke_discount = {}
                                        end
                                        table.insert(source.nuke_discount,target)
                                        target.comms_data.weapon_available.Nuke = true
                                        target.comms_data.weapons["Nuke"] = "neutral"
                                        target.comms_data.max_weapon_refill_amount.neutral = 1
                                    ...
                                    elseif improvement == "EMP" then
                                        if source.emp_discount == nil then
                                            source.emp_discount = {}
                                        end
                                        table.insert(source.emp_discount,target)
                                        target.comms_data.weapon_available.EMP = true
                                        target.comms_data.weapons["EMP"] = "neutral"
                                        target.comms_data.max_weapon_refill_amount.neutral = 1
                                    ...
                                    elseif improvement == "Homing" then
                                        if source.homing_discount == nil then
                                            source.homing_discount = {}
                                        end
                                        table.insert(source.homing_discount,target)
                                        target.comms_data.weapon_available.Homing = true
                                        target.comms_data.max_weapon_refill_amount.neutral = 1
                                    ...
                                    elseif improvement == "Mine" then
                                        if source.mine_discount == nil then
                                            source.mine_discount = {}
                                        end
                                        table.insert(source.mine_discount,target)
                                        target.comms_data.weapon_available.Mine = true
                                        target.comms_data.weapons["Mine"] = "neutral"
                                        target.comms_data.max_weapon_refill_amount.neutral = 1
                                    ...
                                    elseif improvement == "HVLI" then
                                        if source.hvli_discount == nil then
                                            source.hvli_discount = {}
                                        end
                                        table.insert(source.hvli_discount,target)
                                        target.comms_data.weapon_available.HVLI = true
                                        target.comms_data.max_weapon_refill_amount.neutral = 1
...
function requestReinforcements(calling_function)
    ...
    addCommsReply(tableSelectRandom(send_reinforcements_prompt),function()
        ...
        if target.comms_data.reinforcement_cost == nil then
            print("no reinforcement cost!")
        end
        if target.comms_data.service_available == nil then
            target.comms_data.service_available = {}
        end
        local reinforcement_info = {}
        for template, cost in pairs(target.comms_data.reinforcement_cost) do
            target.comms_data.service_available[template] = random(1,100) < 72
            table.insert(reinforcement_info, {
                desc = _("stationAssist-comms",template),
                template = template,
                threshold = target.comms_data.reinforcement_threshold[template],
                cost = math.ceil(target.comms_data.reinforcement_cost[template]),
                avail = target.comms_data.service_available[template]
            })
        end
        local avail_count = 0
        for i, info in ipairs(reinforcement_info) do
            if info.avail and target.comms_data.friendlyness > info.threshold then
                avail_count = avail_count + 1
                addCommsReply(string.format(_("stationAssist-comms","%s (%d reputation)"),info.desc,info.cost), function()
                    if source:getWaypointCount() < 1 then
                        ...
                        for n = 1, source:getWaypointCount() do
                            addCommsReply(string.format(_("stationAssist-comms", "Waypoint %d"), n),function()
                                if source:takeReputationPoints(info.cost) then
                                    local ship = CpuShip():setFactionId(target:getFactionId()):setPosition(target:getPosition()):setTemplate(info.template):setScanned(true):orderDefendLocation(source:getWaypoint(n))
                                    suffix_index = math.random(11,77)
                                    ship:setCallSign(generateCallSign(nil,target:getFaction()))
...
function requestSupplyDrop(calling_function)
    ...
    addCommsReply(tableSelectRandom(supply_drop_request),function()
            ...
            if source:getWaypointCount() < 1 then
                ...
                for n=1,source:getWaypointCount() do
                    addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                        if source:takeReputationPoints(getServiceCost("supplydrop")) then
                            local position_x, position_y = target:getPosition()
                            local target_x, target_y = source:getWaypoint(n)
                            local script = Script()
                            script:setVariable("position_x", position_x):setVariable("position_y", position_y)
                            script:setVariable("target_x", target_x):setVariable("target_y", target_y)
                            script:setVariable("faction_id", source:getFactionId()):run("supply_drop.lua")
        ...
        if target.comms_data.friendlyness > 20 then
            ...
            addCommsReply(tableSelectRandom(jump_drop_cost),function()
                if source:getWaypointCount() < 1 then
                    ...
                    for n=1,source:getWaypointCount() do
                        addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                            if source:takeReputationPoints(getServiceCost("jumpsupplydrop")) then
                                local position_x, position_y = target:getPosition()
                                local target_x, target_y = source:getWaypoint(n)
                                local script = Script()
                                script:setVariable("position_x", position_x):setVariable("position_y", position_y)
        ...
        if target.comms_data.friendlyness > 66 then
            ...
            addCommsReply(tableSelectRandom(flinger_drop_cost),function()
                ...
                addCommsReply(tableSelectRandom(standard_only),function()
                    if source:getWaypointCount() < 1 then
                        ...
                        for n=1,source:getWaypointCount() do
                            addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                if source:takeReputationPoints(getServiceCost("flingsupplydrop")) then
                                    local target_x, target_y = source:getWaypoint(n)
                                    ...
                                    local sd = SupplyDrop():setFactionId(target:getFactionId()):setPosition(target_x + landing_x, target_y + landing_y):setEnergy(500):setWeaponStorage("Nuke", 1):setWeaponStorage("Homing", 4):setWeaponStorage("Mine", 2):setWeaponStorage("EMP", 1)
                ...
                addCommsReply(string.format(_("stationAssist-comms","Add HVLIs (%d rep + %d rep = %d rep)"),getServiceCost("flingsupplydrop"),getWeaponCost("HVLI")*5,getServiceCost("flingsupplydrop") + (getWeaponCost("HVLI")*5)),function()
                    if source:getWaypointCount() < 1 then
                        ...
                        for n=1,source:getWaypointCount() do
                            addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                if source:takeReputationPoints(getServiceCost("flingsupplydrop") + (getWeaponCost("HVLI")*5)) then
                                    local target_x, target_y = source:getWaypoint(n)
                                    local target_angle = random(0,360)
                                    local flinger_miss = random(100,5000)
                                    local landing_x, landing_y = vectorFromAngle(target_angle,flinger_miss)
                                    local sd = SupplyDrop():setFactionId(target:getFactionId()):setPosition(target_x + landing_x, target_y + landing_y):setEnergy(500):setWeaponStorage("HVLI",5):setWeaponStorage("Nuke", 1):setWeaponStorage("Homing", 4):setWeaponStorage("Mine", 2):setWeaponStorage("EMP", 1)
                                    local supply_location = {
                                        string.format("Supplies delivered %.1f units from waypoint, bearing %.1f.",flinger_miss/1000,target_angle),
                                        string.format("Supplies have been launched. You can find them %.1f units from waypoint %i on bearing %.1f",flinger_miss/1000,n,target_angle),
                                        string.format("Our flinger has launched your supplies at waypoint %i. Look for them at %.1f units from waypoint, bearing %.1f",n,flinger_miss/1000,target_angle),
                ...
                addCommsReply(string.format(_("stationAssist-comms","Add hull repair (%d rep + %d rep = %d rep)"),getServiceCost("flingsupplydrop"),100,getServiceCost("flingsupplydrop") + 100),function()
                    if source:getWaypointCount() < 1 then
                        ...
                        for n=1,source:getWaypointCount() do
                            addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                if source:takeReputationPoints(getServiceCost("flingsupplydrop") + 100) then
                                    local target_x, target_y = source:getWaypoint(n)
                                    local target_angle = random(0,360)
                                    local flinger_miss = random(100,5000)
                                    local landing_x, landing_y = vectorFromAngle(target_angle,flinger_miss)
                ...
                addCommsReply(string.format(_("stationAssist-comms","Add probes (%d rep + %d rep = %d rep)"),getServiceCost("flingsupplydrop"),20,getServiceCost("flingsupplydrop") + 20),function()
                    if source:getWaypointCount() < 1 then
                        ...
                        for n=1,source:getWaypointCount() do
                            addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                if source:takeReputationPoints(getServiceCost("flingsupplydrop") + 20) then
                                    local target_x, target_y = source:getWaypoint(n)
                                    local target_angle = random(0,360)
                                    local flinger_miss = random(100,5000)
                                    local landing_x, landing_y = vectorFromAngle(target_angle,flinger_miss)
                ...
                if target.comms_data.available_repair_crew == nil then
                    target.comms_data.available_repair_crew = math.random(0,5)
                    target.comms_data.available_repair_crew_cost_friendly_needy_over_66 = math.random(30,60)
                    target.comms_data.available_repair_crew_cost_neutral_needy_over_66 = math.random(45,90)
                    target.comms_data.available_repair_crew_cost_excess = math.random(15,30)
                    target.comms_data.available_repair_crew_cost_under_66 = math.random(15,30)
                end
                if target.comms_data.available_repair_crew > 0 then
                    local hire_cost = 0
                    if source:isFriendly(target) then
                        hire_cost = target.comms_data.available_repair_crew_cost_friendly_needy_over_66
                    else
                        hire_cost = target.comms_data.available_repair_crew_cost_neutral_needy_over_66
                    end
                    if target.comms_data.friendlyness <= 66 then
                        hire_cost = hire_cost + target.comms_data.available_repair_crew_cost_under_66
                    end
                    if source:getRepairCrewCount() >= source.maxRepairCrew then
                        hire_cost = hire_cost + target.comms_data.available_repair_crew_cost_excess
                    end
                        ...
                        if source:getWaypointCount() < 1 then
                            ...
                            for n=1,source:getWaypointCount() do
                                addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                    ...
                                    if source:takeReputationPoints(getServiceCost("flingsupplydrop") + hire_cost) then
                                        local target_x, target_y = source:getWaypoint(n)
                                        local target_angle = random(0,360)
                                        local flinger_miss = random(100,5000)
                                        local landing_x, landing_y = vectorFromAngle(target_angle,flinger_miss)
                                        local sd = SupplyDrop():setFactionId(target:getFactionId()):setPosition(target_x + landing_x, target_y + landing_y):setEnergy(500):setWeaponStorage("Nuke", 1):setWeaponStorage("Homing", 4):setWeaponStorage("Mine", 2):setWeaponStorage("EMP", 1)
                                        target.comms_data.available_repair_crew = target.comms_data.available_repair_crew - 1
                                        sd:onPickUp(function(self,player)
                                            string.format("")
                ...
                if target.comms_data.coolant_inventory == nil then
                    target.comms_data.coolant_inventory = math.random(0,5)*2
                    target.comms_data.coolant_inventory_cost_friendly_needy_over_66 = math.random(30,60)
                    target.comms_data.coolant_inventory_cost_neutral_needy_over_66 = math.random(45,90)
                    target.comms_data.coolant_inventory_excess = math.random(15,30)
                    target.comms_data.coolant_inventory_under_66 = math.random(15,30)
                end
                if target.comms_data.coolant_inventory > 0 then
                    local coolant_cost = 0
                    if source:isFriendly(target) then
                        coolant_cost = target.comms_data.coolant_inventory_cost_friendly_needy_over_66
                    else
                        coolant_cost = target.comms_data.coolant_inventory_cost_neutral_needy_over_66
                    end
                    if target.comms_data.friendlyness <= 66 then
                        coolant_cost = coolant_cost + target.comms_data.coolant_inventory_under_66
                    end
                    if source:getMaxCoolant() >= source.initialCoolant then
                        coolant_cost = coolant_cost + target.comms_data.coolant_inventory_excess
                    end
                        ...
                        if source:getWaypointCount() < 1 then
                            ...
                            for n=1,source:getWaypointCount() do
                                addCommsReply(string.format(_("stationAssist-comms","Waypoint %i"),n), function()
                                    ...
                                    if source:takeReputationPoints(getServiceCost("flingsupplydrop") + coolant_cost) then
                                        local target_x, target_y = source:getWaypoint(n)
                                        local target_angle = random(0,360)
                                        local flinger_miss = random(100,5000)
                                        local landing_x, landing_y = vectorFromAngle(target_angle,flinger_miss)
                                        local sd = SupplyDrop():setFactionId(target:getFactionId()):setPosition(target_x + landing_x, target_y + landing_y):setEnergy(500):setWeaponStorage("Nuke", 1):setWeaponStorage("Homing", 4):setWeaponStorage("Mine", 2):setWeaponStorage("EMP", 1)
                                        target.comms_data.coolant_inventory = target.comms_data.coolant_inventory - 2
                                        sd:onPickUp(function(self,player)
                                            string.format("")
...
function requestJonque()
    ...
    addCommsReply(tableSelectRandom(send_jonque), function()
        ...
        local jonque_direct_to_ship = {
            "Direct",
            string.format("Go directly to %s",source:getCallSign()),
            "Meet us directly",
            string.format("Meet %s directly",source:getCallSign()),
        }
        addCommsReply(tableSelectRandom(jonque_direct_to_ship),function()
            if source:takeReputationPoints(getServiceCost("servicejonque")) then
                ship = serviceJonque(target:getFaction()):setPosition(target:getPosition()):setCallSign(generateCallSign(nil,target:getFaction())):setScanned(true):orderDefendTarget(source)
                ship.comms_data = {
                    friendlyness = random(0.0, 100.0),
                    weapons = {
                        Homing = target.comms_data.weapons.Homing,
                        HVLI = target.comms_data.weapons.HVLI,
                        Mine = target.comms_data.weapons.Mine,
                        Nuke = target.comms_data.weapons.Nuke,
                        EMP = target.comms_data.weapons.EMP,
                    },
                    weapon_cost = {
                        Homing = target.comms_data.weapon_cost.Homing * 2,
                        HVLI = target.comms_data.weapon_cost.HVLI * 2,
                        Mine = target.comms_data.weapon_cost.Mine * 2,
                        Nuke = target.comms_data.weapon_cost.Nuke * 2,
                        EMP = target.comms_data.weapon_cost.EMP * 2,
                    },
                    weapon_inventory = {
                        Homing = 40,
                        HVLI = 40,
                    ...
                    reputation_cost_multipliers = {
                        friend = target.comms_data.reputation_cost_multipliers.friend,
                        neutral = math.max(target.comms_data.reputation_cost_multipliers.friend,target.comms_data.reputation_cost_multipliers.neutral/2)
                    },
                }
        ...
        if source:getWaypointCount() < 1 then
            ...
            for n=1,source:getWaypointCount() do
                ...
                addCommsReply(tableSelectRandom(rendezvous_prompts),function()
                    if source:takeReputationPoints(getServiceCost("servicejonque")) then
                        ship = serviceJonque(target:getFaction()):setPosition(target:getPosition()):setCallSign(generateCallSign(nil,target:getFaction())):setScanned(true):orderDefendLocation(source:getWaypoint(n))
                        ship.comms_data = {
                            friendlyness = random(0.0, 100.0),
                            weapons = {
                                Homing = target.comms_data.weapons.Homing,
...
function requestExpediteDock(calling_function)
    ...
    addCommsReply(tableSelectRandom(expedite_dock_prompts),function()
        if source.expedite_dock == nil then
            local explain_expedite_dock = {
                ...
                string.format("We can direct dock workers to be ready to service your docked ship. They won't wait for long. How long before you dock with %s?",target:getCallSign()),
                ...
                string.format("To reduce time spent docked at %s, we can hire dock workers to rapidly load and service %s as soon as you dock. However, we can only hire them for a limited period of time. When are you docking?",target:getCallSign(),source:getCallSign()),
            }
            ...
            addCommsReply(tableSelectRandom(short_prompts),function()
                if source:takeReputationPoints(short_reputation) then
                    source.expedite_dock = {["limit"] = short_minutes*60}
                    setExpediteDock()
                else
                    local insufficient_rep_responses = {
                        "Insufficient reputation",
            ...
            addCommsReply(tableSelectRandom(medium_prompts),function()
                if source:takeReputationPoints(medium_reputation) then
                    source.expedite_dock = {["limit"] = medium_minutes*60}
                    setExpediteDock()
                else
                    local insufficient_rep_responses = {
                        "Insufficient reputation",
            ...
            addCommsReply(tableSelectRandom(long_prompts),function()
                if source:takeReputationPoints(long_reputation) then
                    source.expedite_dock = {["limit"] = long_minutes*60}
                    setExpediteDock()
                else
                    local insufficient_rep_responses = {
                        "Insufficient reputation",
...
function setExpediteDock()
    ...
    if source.expedite_dock == nil then
        ...
        if source.expedite_dock.limit == nil then
            setCommsMessage(_("station-comms","Communications glitch. Please try again."))
            addCommsReply(_("Back"), commsStation)
        else
            local out = ""
            if source.expedite_dock.expire == nil then
                source.expedite_dock.expire = getScenarioTime() + source.expedite_dock.limit
                if expedite_dock_players == nil then
                    expedite_dock_players = {}
                end
                expedite_dock_players[source] = true
                source.expedite_dock.station = target
                local standard_service_count = 0
                if target:getSharesEnergyWithDocked() then
                    source.expedite_dock.energy = true
                    standard_service_count = standard_service_count + 1
                    out = _("station-comms","energy")
                end
                if target:getRepairDocked() then
                    source.expedite_dock.hull = true
                    standard_service_count = standard_service_count + 1
                    if out == "" then
                ...
                if target:getRestocksScanProbes() then
                    source.expedite_dock.probes = true
                    standard_service_count = standard_service_count + 1
                    if out == "" then
                ...
                if source.expedite_dock.station ~= target then
                    if source.expedite_dock.station:isValid() then
                        local what_about_current_contract = {
                            string.format("I represent station %s.\nI see that you have an expedited docking contract with station %s.\nWould you like to cancel it?",target:getCallSign(),source.expedite_dock.station:getCallSign()),
                            string.format("Considering an expedited docking contract with %s, eh? What should be done about your existing expedited contract with %s? Cancel it?",target:getCallSign(),source.expedite_dock.station:getCallSign()),
                            string.format("Want a fast dock with %s? What should be done about your current agreement with %s? Should it be cancelled?",target:getCallSign(),source.expedite_dock.station:getCallSign()),
                            string.format("You can't set up a quick dock with %s until your quick dock with %s is done or cancelled. Shall I cancel it?",target:getCallSign(),source.expedite_dock.station:getCallSign()),
                        }
                        setCommsMessage(tableSelectRandom(what_about_current_contract))
                        local cancel_fast_dock_prompt = {
                            string.format("Yes, cancel expedited docking contract with %s",source.expedite_dock.station:getCallSign()),
                            string.format("Abandon fast dock plan with %s",source.expedite_dock.station:getCallSign()),
                            string.format("Cancel planned quick dock with %s",source.expedite_dock.station:getCallSign()),
                            string.format("Please cancel the fast dock contract with %s",source.expedite_dock.station:getCallSign()),
                        }
                        addCommsReply(tableSelectRandom(cancel_fast_dock_prompt),function()
                            local fast_dock_contract_cancelled = {
                                string.format("Expedited docking contract with %s has been cancelled.",source.expedite_dock.station:getCallSign()),
                                string.format("Fast dock cancelled with %s",source.expedite_dock.station:getCallSign()),
                                string.format("Ok, we just cancelled your expedited docking contract with %s",source.expedite_dock.station:getCallSign()),
                                string.format("%s fast dock contract cancelled",source.expedite_dock.station:getCallSign()),
                            }
                            setCommsMessage(tableSelectRandom(fast_dock_contract_cancelled))
                            expedite_dock_players[source] = nil
                            source.expedite_dock = nil
                            addCommsReply(_("Back"), commsStation)
                        end)
                        local keep_fast_dock_contract = {
                            string.format("No, keep existing expedited docking contract with %s",source.expedite_dock.station:getCallSign()),
                            string.format("Oops, I forgot about that. I need to keep the fast dock contract with %s",source.expedite_dock.station:getCallSign()),
                            string.format("I'd better keep the existing quick dock contract with %s",source.expedite_dock.station:getCallSign()),
                            string.format("Keep the fast dock plan with %s. Let's not waste the reputation already spent there",source.expedite_dock.station:getCallSign()),
                        }
                        addCommsReply(tableSelectRandom(keep_fast_dock_contract),function()
                            local fast_dock_contract_kept = {
                                string.format("Ok, we left the fast dock contract in place with %s",source.expedite_dock.station:getCallSign()),
                                string.format("Kept the quick dock contract with %s",source.expedite_dock.station:getCallSign()),
                                string.format("The expedited dock contract with %s remains in effect",source.expedite_dock.station:getCallSign()),
                                string.format("Maintaining the fast dock contract with %s",source.expedite_dock.station:getCallSign()),
                            }
            ...
            if source.expedite_dock.station == target then
                ...
                if out == "" then
                    local minutes = 0
                    local seconds = source.expedite_dock.expire - getScenarioTime()
                    if seconds > 60 then
                        minutes = seconds / 60
                        seconds = seconds % 60
                        out = string.format(_("station-comms","Expected dock with %s in %i:%.2i"),target:getCallSign(),math.floor(minutes),math.floor(seconds))
                    else
                        out = string.format(_("station-comms","Expected dock with %s in 0:%.2i"),target:getCallSign(),math.floor(seconds))
                    end
                end
                service_list = _("station-comms","Expedited service list:")
                if source.expedite_dock.energy then
                    ...
                    addCommsReply(tableSelectRandom(replenish_energy_fast_dock_prompt),function()
                        if source:takeReputationPoints(5) then
                            source.expedite_dock.energy = true
                            setExpediteDock()
                        else
                            local insufficient_rep_responses = {
                ...
                if source.expedite_dock.hull then
                    ...
                    addCommsReply(tableSelectRandom(repair_hull_fast_dock_prompt),function()
                        if source:takeReputationPoints(10) then
                            source.expedite_dock.hull = true
                            setExpediteDock()
                        else
                            local insufficient_rep_responses = {
                ...
                if source.expedite_dock.probes then
                    if service_list == _("station-comms","Expedited service list:") then
                ...
                if source.expedite_dock.nuke ~= nil then
                    if service_list == _("station-comms","Expedited service list:") then
                        if source.expedite_dock.nuke > 1 then
                            service_list = string.format(_("station-comms","%s %s nukes"),service_list,source.expedite_dock.nuke)
                        else
                    ...
                    if target.comms_data.weapon_available.Nuke and isAllowedTo(target.comms_data.weapons.Nuke) then
                        local max_nuke = source:getWeaponStorageMax("Nuke")
                        if max_nuke > 0 then
                            local current_nuke = source:getWeaponStorage("Nuke")
                            if current_nuke < max_nuke then
                                ...
                                addCommsReply(tableSelectRandom(replenish_nukes_fast_dock_prompt),function()
                                    if source:takeReputationPoints(getWeaponCost("Nuke")*full_nuke) then
                                        source.expedite_dock.nuke = full_nuke
                                        setExpediteDock()
                                    else
                                        local insufficient_rep_responses = {
                                            "Insufficient reputation",
                ...
                if source.expedite_dock.homing ~= nil then
                    if service_list == _("station-comms","Expedited service list:") then
                        if source.expedite_dock.homing > 1 then
                            service_list = string.format(_("station-comms","%s %s homing missiles"),service_list,source.expedite_dock.homing)
                        else
                    ...
                    if target.comms_data.weapon_available.Homing and isAllowedTo(target.comms_data.weapons.Homing) then
                        local max_homing = source:getWeaponStorageMax("Homing")
                        if max_homing > 0 then
                            local current_homing = source:getWeaponStorage("Homing")
                            if current_homing < max_homing then
                                ...
                                addCommsReply(tableSelectRandom(refill_homing_fast_dock_prompt),function()
                                    if source:takeReputationPoints(getWeaponCost("Homing")*full_homing) then
                                        source.expedite_dock.homing = full_homing
                                        setExpediteDock()
                                    else
                                        local insufficient_rep_responses = {
                                            "Insufficient reputation",
                ...
                if source.expedite_dock.emp ~= nil then
                    if service_list == _("station-comms","Expedited service list:") then
                        if source.expedite_dock.emp > 1 then
                            service_list = string.format(_("station-comms","%s %s EMP missiles"),service_list,source.expedite_dock.emp)
                        else
                    ...
                    if target.comms_data.weapon_available.EMP and isAllowedTo(target.comms_data.weapons.EMP) then
                        local max_emp = source:getWeaponStorageMax("EMP")
                        if max_emp > 0 then
                            local current_emp = source:getWeaponStorage("EMP")
                            if current_emp < max_emp then
                                ...
                                addCommsReply(tableSelectRandom(restock_emp_fast_dock_prompt),function()
                                    if source:takeReputationPoints(getWeaponCost("EMP")*full_emp) then
                                        source.expedite_dock.emp = full_emp
                                        setExpediteDock()
                                    else
                                        local insufficient_rep_responses = {
                                            "Insufficient reputation",
                ...
                if source.expedite_dock.mine ~= nil then
                    if service_list == _("station-comms","Expedited service list:") then
                        if source.expedite_dock.mine > 1 then
                            service_list = string.format(_("station-comms","%s %s mines"),service_list,source.expedite_dock.mine)
                        else
                    ...
                    if target.comms_data.weapon_available.Mine and isAllowedTo(target.comms_data.weapons.Mine) then
                        local max_mine = source:getWeaponStorageMax("Mine")
                        if max_mine > 0 then
                            local current_mine = source:getWeaponStorage("Mine")
                            if current_mine < max_mine then
                                ...
                                addCommsReply(tableSelectRandom(restock_mines_fast_dock_prompt),function()
                                    if source:takeReputationPoints(getWeaponCost("Mine")*full_mine) then
                                        source.expedite_dock.mine = full_mine
                                        setExpediteDock()
                                    else
                                        local insufficient_rep_responses = {
                                            "Insufficient reputation",
                ...
                if source.expedite_dock.hvli ~= nil then
                    if service_list == _("station-comms","Expedited service list:") then
                        if source.expedite_dock.hvli > 1 then
                            service_list = string.format(_("station-comms","%s %s HVLI missiles"),service_list,source.expedite_dock.hvli)
                        else
                    ...
                    if target.comms_data.weapon_available.HVLI and isAllowedTo(target.comms_data.weapons.HVLI) then
                        local max_hvli = source:getWeaponStorageMax("HVLI")
                        if max_hvli > 0 then
                            local current_hvli = source:getWeaponStorage("HVLI")
                            if current_hvli < max_hvli then
                                ...
                                addCommsReply(tableSelectRandom(refill_hvli_quick_dock_prompt),function()
                                    if source:takeReputationPoints(getWeaponCost("HVLI")*full_hvli) then
                                        source.expedite_dock.hvli = full_hvli
                                        setExpediteDock()
                                    else
                                        local insufficient_rep_responses = {
                                            "Insufficient reputation",
                ...
                if source.expedite_dock.repair_crew then
                    if service_list == _("station-comms","Expedited service list:") then
                    ...
                    if target.comms_data.available_repair_crew > 0 then    --station has repair crew available
                        local hire_cost = 0
                        if source:isFriendly(target) then
                            hire_cost = target.comms_data.available_repair_crew_cost_friendly_needy_over_66
                        else
                        ...
                        addCommsReply(tableSelectRandom(hire_repair_crew_fast_dock_prompt),function()
                            ...
                            if source:takeReputationPoints(hire_cost) then
                                source.expedite_dock.repair_crew = true
                                setExpediteDock()
                            else
                                local insufficient_rep_responses = {
                                    "Insufficient reputation",
                ...
                if source.expedite_dock.coolant then
                    if service_list == _("station-comms","Expedited service list:") then
                    ...
                    if target.comms_data.coolant_inventory > 0 then
                        ...
                        addCommsReply(tableSelectRandom(get_coolant_fast_dock_prompt),function()
                            ...
                            if source:takeReputationPoints(coolant_cost) then
                                source.expedite_dock.coolant = true
                                setExpediteDock()
                            else
                                local insufficient_rep_responses = {
                                    "Insufficient reputation",
...
function commercialOptions(calling_function)
    ...
    local commercial_options_prompt = {
        string.format("Investigate commercial options at %s",target:getCallSign()),
        string.format("Look into %s commercial options",target:getCallSign()),
        string.format("Check out %s commercial options",target:getCallSign()),
        string.format("Explore commercial options at %s",target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(commercial_options_prompt),function()
        local good_sale_count = 0
        local good_sale_list = ""
        for good, good_data in pairs(target.comms_data.goods) do
        ...
        if good_sale_count > 0 then
            ...
            addCommsReply(tableSelectRandom(buy_prompt),function()
                local sell_header = {
                    ...
                    string.format("List of goods being sold by %s:\n(good name, quantity, reputation cost)",target:getCallSign()),
        ...
        if target.comms_data.buy ~= nil then
            local good_buy_list = ""
            local match_good_buy_list = ""
            for good, price in pairs(target.comms_data.buy) do
                if good_buy_list == "" then
                    good_buy_list = good_desc[good]
                else
                    string.format("%s, %s",good_buy_list,good_desc[good])
                end
                if source.goods ~= nil and source.goods[good] ~= nil and source.goods[good] > 0 then
            ...
            addCommsReply(tableSelectRandom(sell_prompt),function()
                local buy_header = {
                    string.format("Goods station %s will buy (good name, reputation):",target:getCallSign()),
                    string.format("List of goods %s will buy:\n(good name, reputation)",target:getCallSign()),
                    string.format("%s will buy these goods:\n(good name, reputation)",target:getCallSign()),
                    string.format("You can sell these goods to %s:(good name, reputation)",target:getCallSign()),
                }
                ...
                if match_good_buy_list == "" then
                    local no_matching_good_addendum = {
                        ...
                        string.format("You have nothing in your cargo hold that %s wants",target:getCallSign()),
                        string.format("%s is not interested in anything in your cargo hold",target:getCallSign()),
                    }
                    buy_out = string.format("%s\n\n%s",buy_out,tableSelectRandom(no_matching_good_addendum))
                else
                    local matching_good_addendum = {
                        "Matching goods in your cargo hold",
                        string.format("%s would buy these goods",target:getCallSign()),
                        string.format("This cargo matches %s's interests",target:getCallSign()),
                        string.format("%s is interested in this cargo",target:getCallSign()),
                    }
        ...
        if target.comms_data.trade ~= nil then
            if target.comms_data.trade.food then
                trade_good_list = good_desc["food"]
            end
            if target.comms_data.trade.medicine then
            ...
            if target.comms_data.trade.luxury then
                if trade_good_list == "" then
        ...
        if source.transport_mission == nil then
            transport_mission_available = true
        else
            if source.transport_mission.destination == nil or not source.transport_mission.destination:isValid() then
                transport_mission_available = true
            end
        end
        if transport_mission_available and #target.characters > 0 then
            if out == "" then
        ...
        if source.cargo_mission == nil then
            if target.residents ~= nil and #target.residents > 0 then
                cargo_mission_available = true
            end
        else
            if source.cargo_mission.loaded then
                if source.cargo_mission.destination == nil or not source.cargo_mission.destination:isValid() then
                    cargo_mission_available = true
                end
            else
                if source.cargo_mission.origin == nil or not source.cargo_mission.origin:isValid() then
                    cargo_mission_available = true
                end
            end
        end
        if cargo_mission_available then
            local resident_list = ""
            for i,resident in ipairs(target.residents) do
        ...
        addCommsReply(tableSelectRandom(external_commerce),function()
            ...
            if target.comms_data.friendlyness > 66 then
                if target.comms_data.other_station_commerce == nil then
                    local station_pool = {}
                    for i,station in ipairs(regionStations) do
                        if station ~= nil and station:isValid() and station ~= target and not station:isEnemy(source) then
                            local station_type = station:getTypeName()
                    ...
                    target.comms_data.other_station_commerce = {}
                    table.insert(target.comms_data.other_station_commerce,tableSelectRandom(station_pool))
                    if target.comms_data.friendlyness > 70 then
                        table.insert(target.comms_data.other_station_commerce,tableSelectRandom(station_pool))
                    end
                    if target.comms_data.friendlyness > 80 then
                        table.insert(target.comms_data.other_station_commerce,tableSelectRandom(station_pool))
                    end
                    if target.comms_data.friendlyness > 90 then
                        table.insert(target.comms_data.other_station_commerce,tableSelectRandom(station_pool))
                    end
                    if target.comms_data.friendlyness > 95 then
                        table.insert(target.comms_data.other_station_commerce,tableSelectRandom(station_pool))
                    end
                end
                local other_stations = ""
                for i,station in ipairs(target.comms_data.other_station_commerce) do
                ...
                if other_stations == "" then
                    local commercially_clueless = {
                        ...
                        string.format("I only know about %s. Other stations are too far away.",target:getCallSign()),
                        string.format("My knowledge does not extend beyond %s.",target:getCallSign()),
                    }
...
function magnasolHeatDiscussion(calling_function)
    local magnasol_protection_prompt = {
        ...
        string.format("How can I reduce the heat from Magnasol on %s",source:getCallSign()),
        string.format("Magnasol is roasting %s! What can we do?",source:getCallSign()),
    }
    addCommsReply(tableSelectRandom(magnasol_protection_prompt),function()
        ...
        addCommsReply(tableSelectRandom(run_away_prompt),function()
            local distance_explained = {
                ...
                string.format("The closer %s gets to Magnasol, the hotter the heat. Conversely, the farther %s is from Magnasol, the less the heat applies.",source:getCallSign(),source:getCallSign()),
                "The farther away you are from Magnasol, the less its heat impacts your ship systems.",
                string.format("Move %s 100 units or more away from Magnasol and the heat impact goes away. The closer you are to Magnasol, though, the more the heat impacts %s",source:getCallSign(),source:getCallSign()),
            }
        ...
        addCommsReply(tableSelectRandom(get_coolant_additive_prompt),function()
            ...
            addCommsReply(tableSelectRandom(try_additive_prompt),function()
                if target.comms_data.coolant_additive == nil then
                    local duration_list = {
                    ...
                    target.comms_data.coolant_additive = {
                ...
                local coolant_additive_offer = {
                    string.format("We've got a batch of coolant additive that's level %i effective (the higher the level, the better) that'll last %s minutes. We'll provide you some in exchange for %s.",target.comms_data.coolant_additive.effectiveness,target.comms_data.coolant_additive.desc,target.comms_data.coolant_additive.exchange),
                    string.format("We've got some %i level effective coolant additive. The higher the level, the greater the effectiveness. It lasts %s minutes. If you provide us with %s, we'll provide some for you.",target.comms_data.coolant_additive.effectiveness,target.comms_data.coolant_additive.desc,target.comms_data.coolant_additive.exchange),
                    string.format("We'll give you some level %i coolant additive for some %s. It lasts about %s minutes. The greater the level, the more effective the additive, btw.",target.comms_data.coolant_additive.effectiveness,target.comms_data.coolant_additive.exchange,target.comms_data.coolant_additive.desc),
                    string.format("In exchange for %s we can provide our coolant additive which lasts %s minutes and is rated at %i effective (the higher the rating the more effective",target.comms_data.coolant_additive.exchange,target.comms_data.coolant_additive.desc,target.comms_data.coolant_additive.effectiveness),
                }
                ...
                if source:isDocked(target) then
                    ...
                    addCommsReply(tableSelectRandom(get_additive),function()
                        local exchange_good = target.comms_data.coolant_additive.exchange
                        local function injectCoolantAdditive(exchange_good)
                            ...
                            if source.goods ~= nil then
                                if source.goods[exchange_good] ~= nil and source.goods[exchange_good] > 0 then
                                    source.goods[exchange_good] = source.goods[exchange_good] - 1
                                    source.coolant_additive = {
                                        ["effectiveness"] = target.comms_data.coolant_additive.effectiveness,
                                        ["expires"] = target.comms_data.coolant_additive.duration*60 + getScenarioTime(),
                                    }
                                    source.coolant_additive_button_eng = "coolant_additive_button_eng"
                                    source:addCustomButton("Engineering",source.coolant_additive_button_eng,"Check Additive",function()
                                        string.format("")
                                        source.coolant_additive_msg_eng = "coolant_additive_msg_eng"
                                        print("expires:",source.coolant_additive.expires)
                                        if source.coolant_additive.expires > getScenarioTime() then
                                            source:addCustomMessage("Engineering",source.coolant_additive_msg_eng,"Additive has dissipated")
                                            source:removeCustom(source.coolant_additive_button_eng)
                                        else
                                            source:addCustomMessage("Engineering",source.coolant_additive_msg_eng,string.format("Additive functioning.\nEffectiveness level %s.\nExpected remaining duration: %i seconds",source.coolant_additive.effectiveness,math.floor(source.coolant_additive.expires - getScenarioTime())))
                                        end
                                    end,41)
                                    source.coolant_additive_button_plus = "coolant_additive_button_plus"
                                    source:addCustomButton("Engineering+",source.coolant_additive_button_plus,"Check Additive",function()
                                        string.format("")
                                        source.coolant_additive_msg_eng = "coolant_additive_msg_eng"
                                        if source.coolant_additive.expires > getScenarioTime() then
                                            source:addCustomMessage("Engineering+",source.coolant_additive_msg_eng,"Additive has dissipated")
                                            source:removeCustom(source.coolant_additive_button_plus)
                                        else
                                            source:addCustomMessage("Engineering+",source.coolant_additive_msg_eng,string.format("Additive functioning.\nEffectiveness level %s.\nExpected remaining duration: %i seconds",source.coolant_additive.effectiveness,math.floor(source.coolant_additive.expires - getScenarioTime())))
                                        end
                                    end,41)
                        ...
                        if source.coolant_additive == nil then
                            injectCoolantAdditive(exchange_good)
                        else
                            if source.coolant_additive.expires ~= nil and source.coolant_additive.expires < getScenarioTime() then
...
function riptideHossenfelderDiscussion(calling_function)
    local info_on_riptide = {
        ...
        string.format("I need to know about the area around %s",target:getCallSign()),
        string.format("What can you tell me about %s's environment",target:getCallSign()),
    }
        ...
        if stationMarielle ~= nil and stationMarielle:isValid() then
            addCommsReply(string.format("Marielle in %s",stationMarielle:getSectorName()),function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_marielle_prompt),function()
                    ...
                    source:commandAddWaypoint(sx, sy)
                    local marielle_waypoint_set = {
                        string.format("Waypoint %i set on station Marielle",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Marielle for you",source:getWaypointCount()),
                        string.format("Marielle now has waypoint %i set on it",source:getWaypointCount()),
                        string.format("Waypoint %i set for station Marielle",source:getWaypointCount()),
                    }
            ...
            addCommsReply(string.format("Ilorea in %s",stationIlorea:getSectorName()),function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_ilorea_prompt),function()
                    ...
                    local ilorea_waypoint_set = {
                        string.format("Waypoint %i set on station Ilorea",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Ilorea for you",source:getWaypointCount()),
                        string.format("Ilorea now has waypoint %i set on it",source:getWaypointCount()),
                        string.format("Waypoint %i set for station Ilorea",source:getWaypointCount()),
                    }
            ...
            addCommsReply(string.format("Rivelle in %s",stationRivelle:getSectorName()),function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_rivelle_prompt),function()
                    ...
                    local rivelle_waypoint_set = {
                        string.format("Waypoint %i set on station Rivelle",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Rivelle for you",source:getWaypointCount()),
                        string.format("Rivelle now has waypoint %i set on it",source:getWaypointCount()),
                        string.format("Waypoint %i set for station Rivelle",source:getWaypointCount()),
                    }
            ...
            addCommsReply(string.format("Borie in %s",stationBorie:getSectorName()),function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_borie_prompt),function()
                    ...
                    local borie_waypoint_set = {
                        string.format("Waypoint %i set on station Borie",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Borie for you",source:getWaypointCount()),
                        string.format("Borie now has waypoint %i set on it",source:getWaypointCount()),
                        string.format("Waypoint %i set for station Borie",source:getWaypointCount()),
                    }
            ...
            addCommsReply(string.format("Lurive in %s",stationLurive:getSectorName()),function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_lurive_prompt),function()
                    ...
                    local lurive_waypoint_set = {
                        string.format("Waypoint %i set on station Lurive",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Lurive for you",source:getWaypointCount()),
                        string.format("Lurive now has waypoint %i set on it",source:getWaypointCount()),
                        string.format("Waypoint %i set for station Lurive",source:getWaypointCount()),
                    }
            ...
            addCommsReply(string.format("Vilairre currently in %s",stationVilairre:getSectorName()), function()
                ...
                addCommsReply(tableSelectRandom(yes_waypoint_vilairre_prompt),function()
                    ...
                    local vilairre_waypoint_set = {
                        string.format("Waypoint %i set on station Vilairre. Since Vilairre orbits Wilaux, this waypoint will become rapidly outdated",source:getWaypointCount()),
                        string.format("I set waypoint %i on station Vilairre. This waypoint will become rapidly outdated since Vilairre orbits Wilaux.",source:getWaypointCount()),
                        string.format("Vilairre now has waypoint %i set on it. Remember, this waypoint will be outdated shortly since the station is in motion.",source:getWaypointCount()),
                        string.format("Waypoint %i has been set for station Vilairre. Note: the waypoint will not move with Vilairre, so it will soon be inaccurate.",source:getWaypointCount()),
                    }
...
function dockedGreeting(source, target, oMsg)
end

function dockedLightPanic(source, target, oMsg)
    local interactive = false
    local no_relay_panic_responses = {
        "No communication officers available due to station emergency.",
        "Relay officers unavailable during station emergency.",
	...
	local panic_range = target.comms_data.panic_range
    if target:areEnemiesInRange(panic_range*1.5) then
        if target.comms_data.friendlyness > 10 then
            oMsg = string.format(_("station-comms","%s\n%s"),oMsg,tableSelectRandom(no_relay_panic_responses))
        end
    elseif target:areEnemiesInRange(panic_range*2) then
        if target.comms_data.friendlyness > 70 then
    ...
    elseif target:areEnemiesInRange(panic_range*2.5) then
        if target.comms_data.friendlyness > 20 then
            if target.comms_data.friendlyness > 60 then
                local distracted_units_responses = {
                    string.format("Please forgive us if we seem distracted. Our sensors detect enemies within %i units",math.floor(panic_range*2.5/1000)),
                    string.format("Enemies at %i units. Things might get busy soon. Business?",math.floor(panic_range*2.5/1000)),
                    string.format("A busy day here at %s: Enemies are %s units away and my boss is reviewing emergency procedures. I'm a bit distracted.",target:getCallSign(),math.floor(panic_range*2.5/1000)),
                ...
                oMsg = string.format(_("station-comms","%s\n%s"),oMsg,tableSelectRandom(distracted_units_responses))
            elseif target.comms_data.friendlyness > 25 then
                local distracted_responses = {
                    "Please forgive us if we seem distracted. Our sensors detect enemies nearby.",
                    string.format("Enemies are close to %s. We might get busy. Business?",target:getCallSign()),
                    "We're quite busy preparing for enemies: evaluating cross training, checking emergency procedures, etc. I'm a little distracted.",
        ...
        if target:isFriendly(source) then
            interactive = true
        elseif not target:isEnemy(source) then
            if target.comms_data.friendlyness > 15 then
                interactive = true
            end
        end
    end
...
function androidDockedStationCommsMeat()
    ...
    if isAllowedTo(target.comms_data.services.activatedefensefleet) then
        stationDefenseFleet({identifier=androidDockedStationCommsMeat,name="automated station communication"})
    end
    if source.goods ~= nil then
        local good_count = 0
        for good, good_quantity in pairs(source.goods) do
        ...
        if good_count > 0 then
			local deposit_goods_prompt = {
				...
				string.format("Put goods in %s's external storage facility",target:getCallSign()),
			}
    ...
    for good, good_quantity in pairs(source.goods) do
        if good_quantity > 0 then
            ...
            addCommsReply(good_desc[good], function()
                string.format("")
                source.goods[good] = source.goods[good] - 1
                source.cargo = source.cargo + 1
                local want_it = false
                if target.comms_data.buy ~= nil then
                    for good_buy, price in pairs(target.comms_data.buy) do
                        if good == good_buy then
                            target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + price/2)
                            source:addReputationPoints(math.floor(price/2))
                            want_it = true
                            break
                        end
                    end
                ...
                if not want_it then
                    target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(3,9))
                end
                local donated_confirmed = {
                    string.format("One %s donated",good_desc[good]),
                    string.format("We donated one %s to %s",good_desc[good],target:getCallSign()),
                    string.format("We donated a %s",good_desc[good]),
                    string.format("We provided %s with one %s",target:getCallSign(),good_desc[good]),
                }
...
function interactiveDockedStationCommsMeat()
    local help_prompts = {
        ...
        string.format("Go ahead, %s",source:getCallSign()),
        string.format("How can %s serve you today?",target:getCallSign()),
    }
...
--[[RM    if target == stationMonocle then
	...
	if not source:isEnemy(target) then
		if source:isFriendly(target) then
			if source.pods ~= source.max_pods then
				unloadEscapePods()
			end
		else
			if source.pods ~= source.max_pods then
				if target.comms_data.escape_pod_cost == nil then
					target.comms_data.escape_pod_cost = math.random(8,12)
				end
				unloadEscapePods(target.comms_data.escape_pod_cost)
			end
		end
	end
    --]]
...
function stationTalk(calling_function)
    local what_do_you_know_prompts = {
        ...
        "Not sure. Talk to me.",
        "Unsure. Help me out.",
        string.format("Well, what does the %s relay officer know?",target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(what_do_you_know_prompts),function()
        ...
        if target.comms_data.gossip ~= nil then	-- TODO if it should be used, it must be filled
            if target.comms_data.friendlyness > 50 + (difficulty * 15) then
                knowledge_count = knowledge_count + 1
                stationGossip(calling_function)
            end
        end
        if target.comms_data.general ~= nil then
            knowledge_count = knowledge_count + 1
            stationGeneralInformation(calling_function)
        end
        if target.comms_data.history ~= nil then
            knowledge_count = knowledge_count + 1
...
function stationGossip(calling_function)
    ...
    addCommsReply(tableSelectRandom(gossip_prompts), function()
        setCommsMessage(target.comms_data.gossip)
        addCommsReply(string.format("Back to %s",calling_function.name),calling_function.identifier)
        addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
        addCommsReply(_("Back to station communication"), commsStation)
    end)
...
function stationGeneralInformation(calling_function)
    ...
    addCommsReply(tableSelectRandom(general_information_prompts), function()
        setCommsMessage(target.comms_data.general)
        addCommsReply(string.format("Back to %s",calling_function.name),calling_function.identifier)
        addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
        addCommsReply(_("Back to station communication"), commsStation)
    end)
...
function stationHistory(calling_function)
    local history_prompts = {
        ...
        string.format("%s history",target:getCallSign()),
        string.format("Historical information on %s",target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(history_prompts), function()
        setCommsMessage(target.comms_data.history)
        addCommsReply(string.format("Back to %s",calling_function.name),calling_function.identifier)
        addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
        addCommsReply(_("Back to station communication"), commsStation)
    end)
...
function dispatchOffice()
    ...
    if mission_options_presented_count == 0 then
        local no_missions_responses = {
            ...
            string.format("No missions or tasks are available here at %s.",target:getCallSign()),
            string.format("%s has no missions or tasks available.",target:getCallSign()),
...
function createTransportMission()
	local mission_character = tableRemoveRandom(target.characters)
	...
	if mission_character ~= nil then
		...
		if mission_target ~= nil and mission_target ~= target and not source:isEnemy(mission_target) then
			target.transport_mission = {
			...
			table.insert(target.characters, mission_character)
		end
	end
end
function createCargoMission()			
	if target.residents ~= nil then
		local mission_character = tableRemoveRandom(target.residents)
		...
		if mission_character ~= nil then
			...
			if mission_origin ~= nil and mission_origin ~= target and not source:isEnemy(mission_origin) then
				target.cargo_mission = {
					["origin"] = mission_origin,
					["origin_name"] = mission_origin:getCallSign(),
					["destination"] = target,
					["destination_name"] = target:getCallSign(),
					["reward"] = 50,
					["character"] = mission_character,
				}
			else
				table.insert(target.residents, mission_character)
			end
		end
	end
end
...
function finishTransportationsMission()
	local who_destination_prompts = {
		string.format("Deliver %s to %s",source.transport_mission.character.name,target:getCallSign()),
		string.format("Escort %s off of %s",source.transport_mission.character.name,source:getCallSign()),
		string.format("Direct %s off the ship to %s",source.transport_mission.character.name,target:getCallSign()),
		string.format("Inform %s of arrival at %s",source.transport_mission.character.name,target:getCallSign()),
	}
	addCommsReply(tableSelectRandom(who_destination_prompts),function()
		if not source:isDocked(target) then
			local stay_docked_to_disembark = {
				"You need to stay docked for that action.",
				string.format("You need to stay docked for %s to disembark.",source.transport_mission.character.name),
				string.format("You must stay docked long enough for %s to get off of %s on to station %s.",source.transport_mission.character.name,source:getCallSign(),target:getCallSign()),
				string.format("You undocked before %s could get off the ship.",source.transport_mission.character.name),
			}
		...
		local thanks_for_ride_responses = {
			string.format("%s disembarks at %s and thanks you",source.transport_mission.character.name,target:getCallSign()),
			string.format("As %s leaves %s at %s, they turn and say, 'Thanks for the ride.'",source.transport_mission.character.name,source:getCallSign(),target:getCallSign()),
			string.format("%s thanks you as they walk away from %s down the short connecting tunnel to %s.",source.transport_mission.character.name,source:getCallSign(),target:getCallSign()),
			string.format("%s disembarks at %s. You hear, 'I'll miss %s,' as footsteps echo back to %s.",source.transport_mission.character.name,target:getCallSign(),source:getCallSign(),source:getCallSign()),
		}
		setCommsMessage(tableRemoveRandom(thanks_for_ride_responses))
		source:addReputationPoints(source.transport_mission.reward)
		if target.residents == nil then
			target.residents = {}
		end
		table.insert(target.residents,source.transport_mission.character)
		source.transport_mission = nil
		addCommsReply(_("Back"), commsStation)
	end)
end
...
function abortTransportationMission()
	local alternate_disembarkation = {
		string.format("%s disembarks at %s because %s has been destroyed. You receive %s reputation for your efforts.",source.transport_mission.character.name,target:getCallSign(),source.transport_mission.destination_name,math.floor(source.transport_mission.reward/2)),
		string.format("%s leaves %s here at %s due to the destruction of %s. You still get %s reputation.",source.transport_mission.character.name,source:getCallSign(),target:getCallSign(),source.transport_mission.destination_name,math.floor(source.transport_mission.reward/2)),
		string.format("%s, %s's original destination, has been destroyed. %s disembarks here. You get %s reputation for trying.",source.transport_mission.destination_name,source.transport_mission.character.name,source.transport_mission.character.name,math.floor(source.transport_mission.reward/2)),
		string.format("Since %s has been destroyed, %s gets off here at %s. Your reputation goes up by %s.",source.transport_mission.destination_name,source.transport_mission.character.name,target:getCallSign(),math.floor(source.transport_mission.reward/2)),
	}
	source:addToShipLog(tableRemoveRandom(alternate_disembarkation),"Yellow")
	source:addReputationPoints(math.floor(source.transport_mission.reward/2))
	if target.residents == nil then
		target.residents = {}
	end
	table.insert(target.residents,source.transport_mission.character)
...
function offerTransportMission()
	local transport_passenger_prompts = {
		...
		string.format("Transport %s",target.transport_mission.character.name),
		"Take on a passenger",
	}
	addCommsReply(tableSelectRandom(transport_passenger_prompts),function()
		local transport_info = {
			string.format("%s wishes to be transported to %s station %s in sector %s.",target.transport_mission.character.name,target.transport_mission.destination:getFaction(),target.transport_mission.destination_name,target.transport_mission.destination:getSectorName()),
			string.format("%s needs a ride to sector %s, specifically to %s station %s.",target.transport_mission.character.name,target.transport_mission.destination:getSectorName(),target.transport_mission.destination:getFaction(),target.transport_mission.destination_name),
			string.format("%s needs to get to station %s. It's a %s station in sector %s.",target.transport_mission.character.name,target.transport_mission.destination_name,target.transport_mission.destination:getFaction(),target.transport_mission.destination:getSectorName()),
			string.format("Can you take %s to %s station %s in sector %s?",target.transport_mission.character.name,target.transport_mission.destination:getFaction(),target.transport_mission.destination_name,target.transport_mission.destination:getSectorName()),
		}
		local transport_reputation_info = {
			string.format("Transporting %s would increase your reputation by %s.",target.transport_mission.character.name,target.transport_mission.reward),
			string.format("If you take %s to %s, you'd increase your reputation by %s.",target.transport_mission.character.name,target.transport_mission.destination_name,target.transport_mission.reward),
			string.format("You'd get %s reputation if you transported %s.",target.transport_mission.reward,target.transport_mission.character.name),
			string.format("This transportation mission is worth %s reputation.",target.transport_mission.reward),
		}
		...
		local transport_agree_prompts = {
			string.format("Agree to transport %s to %s station %s",target.transport_mission.character.name,target.transport_mission.destination:getFaction(),target.transport_mission.destination_name),
			string.format("Agree to transport mission to %s in %s",target.transport_mission.destination_name,target.transport_mission.destination:getSectorName()),
			string.format("%s will transport %s to %s in %s",source:getCallSign(),target.transport_mission.character.name,target.transport_mission.destination_name,target.transport_mission.destination:getSectorName()),
			string.format("Take on passenger transport mission to %s",target.transport_mission.destination_name),
		}
		addCommsReply(tableSelectRandom(transport_agree_prompts),function()
			if not source:isDocked(target) then 
				local stay_docked_to_embark = {
					"You need to stay docked for that action.",
					string.format("You need to stay docked for %s to board.",source.transport_mission.character.name),
					string.format("You must stay docked long enough for %s to board %s from station %s.",source.transport_mission.character.name,source:getCallSign(),target:getCallSign()),
					string.format("You undocked before %s could come aboard.",source.transport_mission.character.name),
				}
				setCommsMessage(tableSelectRandom(stay_docked_to_embark))
				return mission_options_presented_count
			end
			source.transport_mission = target.transport_mission
			target.transport_mission = nil
			local boarding_confirmation = {
				string.format("You direct %s to guest quarters and say, 'Welcome aboard the %s'",source.transport_mission.character.name,source:getCallSign()),
				string.format("You welcome %s aboard the %s. 'Let me show you our guest quarters.'",source.transport_mission.character.name,source:getCallSign()),
				string.format("%s boards %s. 'Allow me to show you the guest quarters where you will stay for our journey to %s'",source.transport_mission.character.name,source:getCallSign(),source.transport_mission.destination_name),
				string.format("%s is aboard. You show %s to %s's guest quarters.",source.transport_mission.character.name,source.transport_mission.character.name,source:getCallSign()),
			}
		...
		addCommsReply(tableSelectRandom(decline_transportation_prompts),function()
			local refusal_responses = {
				string.format("You tell %s that you cannot take on any transportation missions at this time.",target.transport_mission.character.name),
				string.format("You inform %s that you are unable to transport them at this time.",target.transport_mission.character.name),
				string.format("'Sorry, %s. We can't transport you at this time.'",target.transport_mission.character.name),
				string.format("'%s can't transport you right now, %s. Sorry about that. Good luck.'",source:getCallSign(),target.transport_mission.character.name),
			}
			local mission_gone = {
				"The offer disappears from the message board.",
				"The transport mission offer no longer appears on the message board.",
				string.format("%s removes the transport mission offer from the message board.",target.transport_mission.character.name),
				string.format("%s gestures and the transport mission offer disappears from the message board.",target.transport_mission.character.name),
			}
			if random(1,5) <= 1 then
				setCommsMessage(string.format("%s %s",tableSelectRandom(refusal_responses),tableSelectRandom(mission_gone)))
				table.insert(target.residents, target.transport_mission.character)
				target.transport_mission = nil
			else
...
function finishCargoMission()
	local cargo_delivery_prompts = {
		string.format("Deliver cargo to %s on %s",source.cargo_mission.character.name,target:getCallSign()),
		string.format("Give cargo to %s here on %s",source.cargo_mission.character.name,target:getCallSign()),
		string.format("Offload %s's cargo to station %s",source.cargo_mission.character.name,target:getCallSign()),
		string.format("Unload cargo to %s and inform %s",target:getCallSign(),source.cargo_mission.character.name),
	}
	addCommsReply(tableSelectRandom(cargo_delivery_prompts),function()
		if not source:isDocked(target) then 
			local stay_docked_to_deliver = {
				"You need to stay docked for that action.",
				string.format("You need to stay docked to deliver %s's cargo.",source.cargo_mission.character.name),
				string.format("You must stay docked long enough to unload %s's cargo to %s.",source.cargo_mission.character.name,target:getCallSign()),
				string.format("You undocked before we could deliver cargo for %s.",source.cargo_mission.character.name),
			}
			setCommsMessage(tableSelectRandom(stay_docked_to_deliver))
			return
		end
		local cargo_delivery_confirmation_and_thanks = {
			string.format("%s thanks you for retrieving the cargo.",source.cargo_mission.character.name),
			string.format("%s says, 'Thanks for bringing me my stuff.'",source.cargo_mission.character.name),
			string.format("%s grabs the cargo and waves, clearly happy to have it.",source.cargo_mission.character.name),
			string.format("%s takes receipt of the cargo and is clearly grateful.",source.cargo_mission.character.name),
		}
		setCommsMessage(tableSelectRandom(cargo_delivery_confirmation_and_thanks))
		source:addReputationPoints(source.cargo_mission.reward)
		table.insert(target.characters, source.cargo_mission.character)
		source.cargo_mission = nil
		addCommsReply(_("Back"), commsStation)
	end)
end
...
function abortCargoMission()
	local station_destroyed_mid_mission = {
		...
		string.format("%s requests %s's cargo on behalf of their family. %s has been destroyed. You unload the cargo and post a message of condolences for %s's family. You receive %s reputation and delete the mission from your task list.",target:getCallSign(),source.cargo_mission.character.name,source.cargo_mission.destination_name,source.cargo_mission.character.name,math.floor(source.cargo_mission.reward/2)),
	}
	source:addToShipLog(tableSelectRandom(station_destroyed_mid_mission),"Yellow")
	source:addReputationPoints(math.floor(source.cargo_mission.reward/2))
	source.cargo_mission = nil
end

function pickUpCargoMission()
	local mid_cargo_mission_pickup_prompts = {
		string.format("Pick up cargo for %s",source.cargo_mission.character.name),
		string.format("Get cargo for %s",source.cargo_mission.character.name),
		string.format("Load cargo from %s for %s",target:getCallSign(),source.cargo_mission.character.name),
		string.format("Load cargo on %s for %s",source:getCallSign(),source.cargo_mission.character.name),
	}
	addCommsReply(tableSelectRandom(mid_cargo_mission_pickup_prompts),function()
		if not source:isDocked(target) then
			local stay_docked_to_get_cargo = {
				"You need to stay docked for that action.",
				string.format("You need to stay docked to get %s's cargo.",source.cargo_mission.character.name),
				string.format("You must stay docked long enough to load %s's cargo on %s.",source.cargo_mission.character.name,source:getCallSign()),
				string.format("You undocked before we could load cargo for %s.",source.cargo_mission.character.name),
			}
			setCommsMessage(tableSelectRandom(stay_docked_to_get_cargo))
			return
		end
		local cargo_loaded_confirmation = {
			string.format("The cargo for %s has been loaded on %s.",source.cargo_mission.character.name,source:getCallSign()),
			string.format("%s's cargo has been loaded from %s to %s.",source.cargo_mission.character.name,target:getCallSign(),source:getCallSign()),
			string.format("You take receipt of cargo from %s destined for %s.",target:getCallSign(),source.cargo_mission.character.name),
			string.format("You load %s's cargo from %s",source.cargo_mission.character.name,target:getCallSign()),
		}
		setCommsMessage(tableSelectRandom(cargo_loaded_confirmation))
		source.cargo_mission.loaded = true
		addCommsReply(_("Back"), commsStation)
	end)
end
function failedCargoMission()
	local station_destroyed_before_getting_cargo = {
		string.format("Automated systems on %s have informed you of the destruction of station %s. Your mission to retrieve cargo for %s from %s is no longer valid and has been removed from your mission log.",target:getCallSign(),source.cargo_mission.origin_name,source.cargo_mission.character.name,source.cargo_mission.origin_name),
		string.format("Records on %s inform you that %s has been destroyed. Thus, your cargo retrieval mission for %s is no longer valid. It's been removed from your mission task list.",target:getCallSign(),source.cargo_mission.origin_name,source.cargo_mission.character.name),
		string.format("You see on %s's status board that %s was destroyed. So, you can't pick up %s's cargo. You remove the mission from your task list.",target:getCallSign(),source.cargo_mission.origin_name,source.cargo_mission.character.name),
		string.format("%s informs you that %s was destroyed. This invalidates your mission to get %s's cargo from %s. You delete the mission from your task list and send an explanatory message to %s",target:getCallSign(),source.cargo_mission.origin_name,source.cargo_mission.character.name,source.cargo_mission.origin_name,source.cargo_mission.character.name),
	}
	source:addToShipLog(tableSelectRandom(station_destroyed_before_getting_cargo),"Yellow")
	if source.cargo_mission.destination:isValid() then
		table.insert(source.cargo_mission.destination.residents,source.cargo_mission.character)
	end
	source.cargo_mission = nil
end
...
function offerCargoMission()
	local retrieve_cargo_prompts = {
		"Retrieve Cargo",
		string.format("Retrieve cargo for %s",target.cargo_mission.character.name),
		string.format("Get cargo from %s",target.cargo_mission.origin_name),
		string.format("Get cargo for %s from %s",target.cargo_mission.character.name,target.cargo_mission.origin_name),
	}
	addCommsReply(tableSelectRandom(retrieve_cargo_prompts),function()
		local cargo_parameters = {
			string.format("%s wishes you to pick up cargo from %s station %s in sector %s and deliver it here.",target.cargo_mission.character.name,target.cargo_mission.origin:getFaction(),target.cargo_mission.origin_name,target.cargo_mission.origin:getSectorName()),
			string.format("%s wants to hire you to get cargo from %s station %s in %s and deliver it here (%s).",target.cargo_mission.character.name,target.cargo_mission.origin:getFaction(),target.cargo_mission.origin_name,target.cargo_mission.origin:getSectorName(),target:getCallSign()),
			string.format("Mission: Get cargo from %s station %s in sector %s for %s and bring it back here.",target.cargo_mission.origin:getFaction(),target.cargo_mission.origin_name,target.cargo_mission.origin:getSectorName(),target.cargo_mission.character.name),
			string.format("Task: Get cargo for %s from %s and deliver it here. %s is a %s station in sector %s.",target.cargo_mission.character.name,target.cargo_mission.origin_name,target.cargo_mission.origin_name,target.cargo_mission.origin:getFaction(),target.cargo_mission.origin:getSectorName()),
		}
		local cargo_mission_reputation = {
			string.format("Retrieving and delivering this cargo for %s would increase your reputation by %s.",target.cargo_mission.character.name,target.cargo_mission.reward),
			string.format("Getting this cargo from %s for %s would boost your reputation by %s.",target.cargo_mission.origin_name,target.cargo_mission.character.name,target.cargo_mission.reward),
			string.format("Your reputation would go up by %s if you completed this cargo mission for %s.",target.cargo_mission.reward,target.cargo_mission.character.name),
			string.format("You would get %s reputation for getting cargo from %s for %s",target.cargo_mission.reward,target.cargo_mission.origin_name,target.cargo_mission.character.name),
		}
		...
		local agree_to_cargo_mission = {
			string.format("Agree to retrieve cargo for %s",target.cargo_mission.character.name),
			string.format("Sign up to get cargo for %s",target.cargo_mission.character.name),
			string.format("Take on mission to get cargo for %s",target.cargo_mission.character.name),
			string.format("Inform %s that %s will get their cargo",target.cargo_mission.character.name,source:getCallSign())
		}
		addCommsReply(tableSelectRandom(agree_to_cargo_mission),function()
			if not source:isDocked(target) then 
				local stay_docked_to_start_cargo_mission = {
					"You need to stay docked for that action.",
					string.format("You need to stay docked to agree to get %s's cargo.",source.cargo_mission.character.name),
					string.format("You must stay docked long enough to consent to %s's cargo mission.",source.cargo_mission.character.name),
					string.format("You undocked before we could agree to retrieve cargo for %s.",source.cargo_mission.character.name),
				}
				setCommsMessage(tableSelectRandom(stay_docked_to_start_cargo_mission))
				return
			end
			source.cargo_mission = target.cargo_mission
			source.cargo_mission.loaded = false
			target.cargo_mission = nil
			local cargo_agreement_confirmation = {
				string.format("%s thanks you and contacts station %s to let them know that %s will be picking up the cargo.",source.cargo_mission.character.name,source.cargo_mission.origin_name,source:getCallSign()),
				string.format("%s contacts station %s to let them know that %s will be retrieving %s's cargo.",source.cargo_mission.character.name,source.cargo_mission.origin_name,source:getCallSign(),source.cargo_mission.character.name),
				string.format("%s says, 'Thanks %s. I'll let %s know you're picking up my cargo.'",source.cargo_mission.character.name,source:getCallSign(),source.cargo_mission.origin_name),
				string.format("%s says, 'I'll let %s know you're coming for my cargo. Thank you %s.'",source.cargo_mission.character.name,source.cargo_mission.origin_name,source:getCallSign()),
			}
		...
		addCommsReply(tableSelectRandom(decline_cargo_mission),function()
			local cargo_refusal_responses = {
				string.format("You tell %s that you cannot take on any cargo missions at this time.",target.cargo_mission.character.name),
				string.format("You inform %s that you are unable to get any cargo for them at this time.",target.cargo_mission.character.name),
				string.format("'Sorry, %s. We can't retrieve your cargo at this time.'",target.cargo_mission.character.name),
				string.format("'%s can't get cargo for you you right now, %s. Sorry about that. Good luck.'",source:getCallSign(),target.cargo_mission.character.name),
			}
			local cargo_mission_gone = {
				"The offer disappears from the message board.",
				"The cargo mission offer no longer appears on the message board.",
				string.format("%s removes the cargo retrieval mission offer from the message board.",target.cargo_mission.character.name),
				string.format("%s gestures and the cargo mission offer disappears from the message board.",target.cargo_mission.character.name),
			}
			if random(1,5) <= 1 then
				setCommsMessage(string.format("%s %s",tableSelectRandom(cargo_refusal_responses),tableSelectRandom(cargo_mission_gone)))
				table.insert(target.residents,target.cargo_mission.character)
				target.cargo_mission = nil
			else
...
function transportAndCargoMissions()
    ...
    if source.transport_mission ~= nil then
        if source.transport_mission.destination ~= nil and source.transport_mission.destination:isValid() then
            if source.transport_mission.destination == target then
        ...
        if target.transport_mission == nil then
			createTransportMission()
        else    --transport mission not nil
            if not target.transport_mission.destination:isValid() then
                if target.residents == nil then
                    target.residents = {}
                end
                table.insert(target.residents,target.transport_mission.character)
    ...
    if source.cargo_mission ~= nil then
        if source.cargo_mission.loaded then
            if source.cargo_mission.destination ~= nil and source.cargo_mission.destination:isValid() then
                if source.cargo_mission.destination == target then
					mission_options_presented_count = mission_options_presented_count + 1
					finishCargoMission()
                end
            else
            ...
            if source.cargo_mission.origin ~= nil and source.cargo_mission.origin:isValid() then
                if source.cargo_mission.origin == target then
                    mission_options_presented_count = mission_options_presented_count + 1
					pickUpCargoMission()
                end
            else
        ...
        if target.cargo_mission == nil then
			createCargoMission()
        else    --cargo mission exists
            if not target.cargo_mission.origin:isValid() then
                table.insert(target.residents,target.cargo_mission.character)
                target.cargo_mission = nil
            end
        end
...
function restockShip()
    source.repairCrewCoolantReturn = {identifier=restockShip,name="restock ship"}
    ...
    for i, missile_type in ipairs(missile_types) do
        missilePresence = missilePresence + source:getWeaponStorageMax(missile_type)
    end
    if missilePresence > 0 then
        if     (target.comms_data.weapon_available.Nuke   and source:getWeaponStorageMax("Nuke") > 0)   or 
            (target.comms_data.weapon_available.EMP    and source:getWeaponStorageMax("EMP") > 0)    or 
            (target.comms_data.weapon_available.Homing and source:getWeaponStorageMax("Homing") > 0) or 
            (target.comms_data.weapon_available.Mine   and source:getWeaponStorageMax("Mine") > 0)   or 
            (target.comms_data.weapon_available.HVLI   and source:getWeaponStorageMax("HVLI") > 0)   then
                restockOrdnance()
        end
    end    
    if source:isFriendly(target) then
...
function restockOrdnance()
    local ordnance_restock_prompt = {
        ...
        "Restock ordnance",
        string.format("%s needs more ordnance",source:getCallSign()),
        string.format("Please provide ordnance for %s",source:getCallSign()),
    }
    addCommsReply(tableSelectRandom(ordnance_restock_prompt), function()
        local ordnance_type_specification = {
            ...
            string.format("Exactly what kind of ordnance does %s need?",source:getCallSign()),
            string.format("Identify the ordnance type required for %s",source:getCallSign()),
        }
        ...
        for i, missile_type in ipairs(missile_types) do
            if source:getWeaponStorageMax(missile_type) > 0 and target.comms_data.weapon_available[missile_type] then
...
function handleWeaponRestock(weapon)
    ...
    if not source:isDocked(target) then
        local stay_docked_for_weapons_restock = {
            "You need to stay docked for that action.",
            string.format("You need to stay docked to get weapon restock from %s.",target:getCallSign()),
            string.format("You must stay docked long enough to receive ordnance restock from %s.",target:getCallSign()),
            string.format("You undocked before we could load ordnance from %s.",target:getCallSign()),
        }
    ...
    if not done_with_weapon_restock then
        ...
        local item_amount = math.floor(source:getWeaponStorageMax(weapon) * comms_data.max_weapon_refill_amount[getFriendStatus()]) - source:getWeaponStorage(weapon)
        if item_amount <= 0 then
            if weapon == "Nuke" then
                ...
                local full_on_ordnance = {
                    ...
                    string.format("Sorry, but there is no more space on %s for this ordnance type.",source:getCallSign()),
                }
...
--            addCommsReply(_("Back to station communication"), commsStation)
            ...
            if source:getReputationPoints() > points_per_item * item_amount then
                if source:takeReputationPoints(points_per_item * item_amount) then
                    source:setWeaponStorage(weapon, source:getWeaponStorage(weapon) + item_amount)
                    if source:getWeaponStorage(weapon) == source:getWeaponStorageMax(weapon) then
                        local restocked_on_ordnance = {
                            ...
                            string.format("%s's %s magazine has been fully restocked",source:getCallSign(),weapon),
                ...
                if source:getReputationPoints() > points_per_item then
                    local complete_refill_unavailable = {
                        ...
                        string.format("%i reputation is not enough for a full restock of %s",math.floor(source:getReputationPoints()),weapon),
                    }
                    setCommsMessage(tableSelectRandom(complete_refill_unavailable))
                    local max_affordable = math.floor(source:getReputationPoints()/points_per_item)
                    for i=1,max_affordable do
                        addCommsReply(string.format(_("ammo-comms","Get %i (%i x %i = %i reputation)"),i,i,item_amount,i*item_amount),function()
                            string.format("")
                            if source:takeReputationPoints(i*item_amount) then
                                source:setWeaponStorage(weapon, source:getWeaponStorage(weapon) + i)
                                if source:getWeaponStorage(weapon) == source:getWeaponStorageMax(weapon) then
                                    local restocked_on_selected_ordnance = {
                                        ...
                                        string.format("%s's %s magazine has been restocked as requested (amount:%i)",source:getCallSign(),weapon,i),
...
function getRepairCrewFromStation(relationship)
    ...
    if target.comms_data.available_repair_crew == nil then
        ...
        target.comms_data.available_repair_crew_cost_excess = math.random(15,30)
        target.comms_data.available_repair_crew_cost_under_66 = math.random(15,30)
        print(target:getCallSign(),"available repair crew:",target.comms_data.available_repair_crew)
    end
    if target.comms_data.available_repair_crew > 0 then    --station has repair crew available
        ...
        addCommsReply(tableSelectRandom(get_repair_crew_prompts),function()
            if target.comms_data.crew_available_delay == nil or getScenarioTime() > target.comms_data.crew_available_delay then
                local hire_cost = 0
                if source:isFriendly(target) then
                    hire_cost = target.comms_data.available_repair_crew_cost_friendly_needy_over_66
                else
                ...
                setCommsMessage(tableSelectRandom(consider_repair_crew).." ("..tostring(target.comms_data.available_repair_crew).." available)")
                ...
                addCommsReply(tableSelectRandom(recruit_repair_crew_prompt), function()
                    if not source:isDocked(target) then
                        local stay_docked_to_get_repair_crew = {
                            ...
                            string.format("You must stay docked long enough for your repair crew to board %s",source:getCallSign()),
                            string.format("You undocked before the repair crew you wanted to hire could come aboard from %s",target:getCallSign()),
                        }
                        setCommsMessage(tableSelectRandom(stay_docked_to_get_repair_crew))
                        return
                    end
                    if not source:takeReputationPoints(hire_cost) then
                        local insufficient_rep_responses = {
                            "Insufficient reputation",
                            "Not enough reputation",
                            "You need more reputation",
                        ...
                        source:setRepairCrewCount(source:getRepairCrewCount() + 1)
                        target.comms_data.available_repair_crew = target.comms_data.available_repair_crew - 1
                        if target.comms_data.available_repair_crew <= 0 then
                            target.comms_data.new_repair_crew_delay = getScenarioTime() + random(200,500)
                        end
                        local repair_crew_hired = {
                            "Repair crew member hired",
                            "Repair crew member recruited",
                            string.format("%s has a new repair crew member",source:getCallSign()),
                            string.format("Your new repair crew member boards %s and heads down to damage control",source:getCallSign()),
                        }
                        setCommsMessage(tableSelectRandom(repair_crew_hired))
                        target.comms_data.crew_available_delay_reason = nil
                    end
                    addCommsReply(string.format("Back to %s",source.repairCrewCoolantReturn.name),source.repairCrewCoolantReturn.identifier)
                    addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
                    addCommsReply(_("Back to station communication"), commsStation)
                end)
                target.comms_data.crew_available_delay = getScenarioTime() + random(90,300)
            else
                local delay_reason = {
                ...
                if target.comms_data.crew_available_delay_reason == nil then
                    target.comms_data.crew_available_delay_reason = delay_reason[math.random(1,#delay_reason)]
                end
                local delay_seconds = math.floor(target.comms_data.crew_available_delay - getScenarioTime())
                setCommsMessage(string.format(_("trade-comms","%s %i seconds"),target.comms_data.crew_available_delay_reason,delay_seconds))
            end
            addCommsReply(string.format("Back to %s",source.repairCrewCoolantReturn.name),source.repairCrewCoolantReturn.identifier)
            addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
            addCommsReply(_("Back to station communication"), commsStation)
...
function getCoolantFromStation(relationship)
    ...
    if target.comms_data.coolant_inventory == nil then
        ...
        target.comms_data.coolant_inventory_excess = math.random(15,30)
        target.comms_data.coolant_inventory_under_66 = math.random(15,30)
        print(target:getCallSign(),"coolant inventory:",target.comms_data.coolant_inventory)
    end
    if source.initialCoolant ~= nil and target.comms_data.coolant_inventory > 0 then
        presented_option = true
        local get_coolant_prompts = {
            "Purchase coolant",
            "Get more coolant",
            string.format("Get coolant from %s",target:getCallSign()),
            string.format("Ask for more coolant from %s",target:getCallSign()),
        }
        addCommsReply(tableSelectRandom(get_coolant_prompts),function()
            if target.comms_data.coolant_inventory_delay == nil or getScenarioTime() > target.comms_data.coolant_inventory_delay then
                local coolant_cost = 0
                if source:isFriendly(target) then
                    coolant_cost = target.comms_data.coolant_inventory_cost_friendly_needy_over_66
                else
                ...
                local coolant_banter = {
                    ...
                    string.format("What makes %s so hot that you need more coolant?",source:getCallSign()),
                    string.format("Is %s experiencing drought conditions?",source:getCallSign()),
                }
                ...
                local purchase_coolant_prompts = {
                    ...
                    string.format("Purchase coolant from %s (%i reputation)",target:getCallSign(),coolant_cost),
                    string.format("Get coolant from %s for %i reputation",target:getCallSign(),coolant_cost),
                }
                addCommsReply(tableSelectRandom(purchase_coolant_prompts),function()
                    if not source:isDocked(target) then
                        local stay_docked_to_get_coolant = {
                            ...
                            string.format("You must stay docked long enough for your coolant to be loaded on to %s",source:getCallSign()),
                            string.format("You undocked before the coolant you wanted could be loaded from %s",target:getCallSign()),
                        }
                        setCommsMessage(tableSelectRandom(stay_docked_to_get_coolant))
                        return
                    end
                    if not source:takeReputationPoints(coolant_cost) then
                        local insufficient_rep_responses = {
                            "Insufficient reputation",
                            "Not enough reputation",
                            "You need more reputation",
                        ...
                        source:setMaxCoolant(source:getMaxCoolant() + 2)
                        target.comms_data.coolant_inventory = target.comms_data.coolant_inventory - 2
                        local got_coolant_confirmation = {
                            "Additional coolant purchased",
                            "You got more coolant",
                            string.format("%s has loaded additional coolant onto %s",target:getCallSign(),source:getCallSign()),
                            string.format("%s has provided you with some additional coolant",target:getCallSign()),
                        }
                        setCommsMessage(tableSelectRandom(got_coolant_confirmation))
                        target.comms_data.coolant_delay_reason = nil
                    end
                    addCommsReply(string.format("Back to %s",source.repairCrewCoolantReturn.name),source.repairCrewCoolantReturn.identifier)
                    addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
                    addCommsReply(_("Back to station communication"), commsStation)
                end)
                target.comms_data.coolant_inventory_delay = getScenarioTime() + random(90,300)
            else
                ...
                if target.comms_data.coolant_delay_reason == nil then
                    target.comms_data.coolant_delay_reason = tableSelectRandom(coolant_delay_reason)
                end
                local delay_seconds = math.floor(target.comms_data.coolant_inventory_delay - getScenarioTime())
                setCommsMessage(string.format(_("trade-comms","%s %i seconds"),target.comms_data.coolant_delay_reason,delay_seconds))
            end
            addCommsReply(string.format("Back to %s",source.repairCrewCoolantReturn.name),source.repairCrewCoolantReturn.identifier)
            addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
            addCommsReply(_("Back to station communication"), commsStation)
...
function getReplacementFighterFromStation(relationship)
    local presented_option = false
    if #source.carrier_ship_inventory > 0 then
        local ship_capacity = 0
        local ship_inventory = 0
        local replacement_templates = {}
        for i,carrier_ship in ipairs(source.carrier_ship_inventory) do
        ...
        if ship_capacity > ship_inventory then
            ...
            addCommsReply("Replace fighter",function()
                ...
                if target.replacement_fighters == nil then
                    target.replacement_fighters = {}
                    for i,template in ipairs(replacement_templates) do
                        table.insert(target.replacement_fighters,{template = template, quantity = math.random(1,3), cost = math.floor(playerShipStats[template].strength * random(3,5))})
                    end
                    local template_pool = {}
                    for template,details in pairs(source.carrier_ship_types) do
                        if details.carry then
                            local in_replacement_list = false
                            for i,replacement in ipairs(target.replacement_fighters) do
                    ...
                    if random(1,100) < 63 then
                        ...
                        table.insert(target.replacement_fighters,{template = selected_template, quantity = math.random(1,3), cost = math.floor(playerShipStats[selected_template].strength * random(3,5))})
                ...
                print("replacement fighters at",target:getCallSign())
                for i,replacement in ipairs(target.replacement_fighters) do
                ...
                for i,replacement in ipairs(target.replacement_fighters) do
                    if replacement.quantity > 0 then
                        ...
                        addCommsReply(string.format("%s %i reputation",replacement.template,replacement.cost),function()
                            if source:takeReputationPoints(replacement.cost) then
                                for i,carrier_ship in ipairs(source.carrier_ship_inventory) do
                                    if carrier_ship.state == "deployed" then
                                        if carrier_ship.ship == nil or not carrier_ship.ship:isValid() then
                                            source.carrier_ship_inventory[i] = source.carrier_ship_inventory[#source.carrier_ship_inventory]
                                            source.carrier_ship_inventory[#source.carrier_ship_inventory] = nil
                                            break
                                        end
                                    end
                                end
                                local ship = source.carrier_ship_types[replacement.template]
                                ...
                                table.insert(source.carrier_ship_inventory,{
                                    class = ship.class, 
                                    template = ship.name, 
                                    name = ship_name, 
                                    state = "aboard", 
...
function repairShip()
    ...
    local secondary_system = {
        {prompt = _("stationServices-comms","Repair probe launch system (%s Rep)"),    capable = true,    station_avail = target.comms_data.probe_launch_repair,    cost = target.comms_data.service_cost.probe_launch_repair,    ship_avail = source:getCanLaunchProbe(),        enable = "setCanLaunchProbe",    response = _("stationServices-comms", "Your probe launch system has been repaired.")},
        {prompt = _("stationServices-comms","Repair hacking system (%s Rep)"),        capable = true,    station_avail = target.comms_data.hack_repair,            cost = target.comms_data.service_cost.hack_repair,            ship_avail = source:getCanHack(),                enable = "setCanHack",            response = _("stationServices-comms", "Your hacking system has been repaired.")},
        {prompt = _("stationServices-comms","Repair scanning system (%s Rep)"),        capable = true,    station_avail = target.comms_data.scan_repair,            cost = target.comms_data.service_cost.scan_repair,            ship_avail = source:getCanScan(),                enable = "setCanScan",            response = _("stationServices-comms", "Your scanners have been repaired.")},
        {prompt = _("stationServices-comms","Repair combat maneuver (%s Rep)"),        capable = true,    station_avail = target.comms_data.combat_maneuver_repair,    cost = target.comms_data.service_cost.combat_maneuver_repair,    ship_avail = source:getCanCombatManeuver(),    enable = "setCanCombatManeuver",response = _("stationServices-comms", "Your combat maneuver has been repaired.")},
        {prompt = _("stationServices-comms","Repair self destruct system (%s Rep)"),capable = true,    station_avail = target.comms_data.self_destruct_repair,    cost = target.comms_data.service_cost.self_destruct_repair,    ship_avail = source:getCanSelfDestruct(),        enable = "setCanSelfDestruct",    response = _("stationServices-comms", "Your self destruct system has been repaired.")},
    }
    ...
    if offer_repair then
        ...
        addCommsReply(tableSelectRandom(repair_secondary_prompts),function()
            local which_secondary_system = {
                ...
                string.format("Poor, poor %s. What part of her is hurting now?",source:getCallSign()),
            }
                        ...
                        addCommsReply(string.format(secondary.prompt,secondary.cost),function()
                            if not source:isDocked(target) then
                                local stay_docked_to_get_repaired = {
                                    ...
                                    string.format("You must stay docked long enough for repairs to %s to be completed.",source:getCallSign()),
                                    string.format("You undocked before %s could complete the repairs you wanted.",target:getCallSign()),
                                }
                                setCommsMessage(tableSelectRandom(stay_docked_to_get_repaired))
                                return
                            end
                            if source:takeReputationPoints(secondary.cost) then
                                if secondary.enable == "setCanLaunchProbe" then
                                    source:setCanLaunchProbe(true)
                                elseif secondary.enable == "setCanHack" then
                                    source:setCanHack(true)
                                elseif secondary.enable == "setCanScan" then
                                    source:setCanScan(true)
                                elseif secondary.enable == "setCanCombatManeuver" then
                                    source:setCanCombatManeuver(true)
                                elseif secondary.enable == "setCanSelfDestruct" then
                                    source:setCanSelfDestruct(true)
                                end
    ...
    offer_repair = false
    if target.comms_data.system_repair ~= nil then
        for i, system in ipairs(system_list) do
            if source:hasSystem(system) then
                if source:getSystemHealthMax(system) < 1 then
                    if target.comms_data.system_repair[system].avail then
                        if target.comms_data.system_repair[system].cost > 0 then
                            if target.player_system_repair_service == nil then
                                offer_repair = true
                                table.insert(system_repair_list,system)
                            else
                                if target.player_system_repair_service[source] == nil then
                                    offer_repair = true
                                    table.insert(system_repair_list,system)
                                else
                                    if target.player_system_repair_service[source][system] == nil then
                                        offer_repair = true
                                        table.insert(system_repair_list,system)
                                    end
                                end
    ...
    if offer_repair then
        ...
        local primary_repair_prompt = {
            ...
            string.format("Fix primary system on %s",source:getCallSign()),
        ...
        addCommsReply(tableSelectRandom(primary_repair_prompt),function()
            local what_primary_system = {
                ...
                string.format("What severe wounds on %s can %s help heal?",source:getCallSign(),target:getCallSign()),
                string.format("What primary ship system can %s work on to bring %s back into good working order?",target:getCallSign(),source:getCallSign()),
            }
            setCommsMessage(what_primary_system[math.random(1,#what_primary_system)])
            for index, system in ipairs(system_repair_list) do
                addCommsReply(string.format(_("stationServices-comms","Repair %s max health up to %.1f%% (%i rep)"),pretty_system[system],target.comms_data.system_repair[system].max*100,target.comms_data.system_repair[system].cost), function()
                    if source:takeReputationPoints(target.comms_data.system_repair[system].cost) then
                        if target.player_system_repair_service == nil then
                            target.player_system_repair_service = {}
                        end
                        if target.player_system_repair_service[source] == nil then
                            target.player_system_repair_service[source] = {}
                        end
                        target.player_system_repair_service[source][system] = true
                        local working_on_system = {
                            string.format("We'll start working on your %s maximum health right away.",pretty_system[system]),
                            string.format("We will put %s repair technicians to work on your %s maximum health immediately.",target:getCallSign(),pretty_system[system]),
                            string.format("%s has put repair technicians to work on %s's %s maximum health.",target:getCallSign(),source:getCallSign(),pretty_system[system]),
    ...
    if options_presented_count == 0 then
        local no_applicable_repair_service = {
            ...
            string.format("%s has no repair service that %s can use",target:getCallSign(),source:getCallSign()),
            "There's no repair service here that applies to your ship",
            string.format("There's nothing on %s that %s can repair",source:getCallSign(),target:getCallSign()),
        }
...
function enhanceShip()
    source.repairCrewCoolantReturn = {identifier=enhanceShip,name="enhance ship"}
...
--[[RM
	if target.comms_data.sensor_boost ~= nil then
        if target.comms_data.sensor_boost.cost > 0 then
            boostSensorsWhileDocked()
            presented_option = true
        end
    end
    if source.security_morale < 1 then
        if #security_morale_boosters > 0 then
            if target.security_training == nil then
                target.security_training = (random(1,100) < 77)
            end
            if target.security_training then
                increaseSecurityMorale()
                presented_option = true
            end
        end
    end
    if target.comms_data.fast_probes ~= nil or target.comms_data.remote_warp_jammer ~= nil or target.comms_data.sensor_boost_probes ~= nil or target.comms_data.mine_probes ~= nil then
        if (target.comms_data.fast_probes ~= nil and target.comms_data.fast_probes.quantity > 0) or 
           (target.comms_data.remote_warp_jammer ~= nil and target.comms_data.remote_warp_jammer.quantity > 0) or
           (target.comms_data.sensor_boost_probes ~= nil and target.comms_data.sensor_boost_probes.quantity > 0) or
           (target.comms_data.mine_probes ~= nil and target.comms_data.mine_probes.quantity > 0) then
                   getSpecialtyProbes()
                   presented_option = true
        end
    end
    ...
    if not presented_option then
        local no_applicable_enhancements = {
            ...
            string.format("%s has no ship enhancements available for %s",target:getCallSign(),source:getCallSign()),
            string.format("%s cannot benefit from any ship enhancement on %s",source:getCallSign(),target:getCallSign()),
            string.format("No ship enhancements available for %s on %s",source:getCallSign(),target:getCallSign()),
        }
...
function presentProximityScanner()
    ...
    addCommsReply(tableSelectRandom(proximity_scanner_prompt),function()
        local proximity_scanner_explained = {
            string.format("We've got this portable automatic proximity scanner here. They are very popular. It automatically performs a simple scan on ships in range (%iU). Would you like to have this installed?",target.proximity_scanner_range),
            string.format("We have an automatic proximity scanner that we are not using. These things are pretty popular right now. When a ship gets in range (%iU), it automatically and instantly performs a simple scan on the ship. Would you like for us to install it on %s?",target.proximity_scanner_range,source:getCallSign()),
            string.format("Available for a limited time, we have the ever popular automatic proximity scanner. Install this baby and ships are instantly and automatically simple scanned when they get in range (%iU). Do you want it installed?",target.proximity_scanner_range),
            string.format("The %s quartermaster tells me that there's a spare automatic proximity scanner without a ship designated for installation. These automated proximity scanners are very popular. They instantly and automatically scan ships that are in range (%iU). Would you like it installed on %s?",target:getCallSign(),target.proximity_scanner_range,source:getCallSign()),
        }
        ...
        local install_proximity_scanner = {
            string.format("We'll take it (%i reputation)",target.proximity_scanner_range * price_per_range_unit),
            string.format("Install it, please (%i reputation)",target.proximity_scanner_range * price_per_range_unit),
            string.format("It's perfect! Install it (%i reputation)",target.proximity_scanner_range * price_per_range_unit),
            string.format("We could use that. Please install it (%i reputation)",target.proximity_scanner_range * price_per_range_unit),
        }
        addCommsReply(tableSelectRandom(install_proximity_scanner),function()
            if source:takeReputationPoints(target.proximity_scanner_range * price_per_range_unit) then
                local temp_prox_scan = source.prox_scan
                source.prox_scan = target.proximity_scanner_range
                if temp_prox_scan ~= nil and temp_prox_scan > 0 then
                    target.proximity_scanner_range = temp_prox_scan
                else
                    target.proximity_scanner = false
                    target.proximity_scanner_range = nil
                end
                local proximity_scanner_installed_confirmation = {
                    "Installed",
                    string.format("%s has installed the automatic proximity scanner",target:getCallSign()),
                    "It's installed",
                    string.format("%s now has an automatic proximity scanner",source:getCallSign()),
                }
...
function presentMaxHealthWidgets()
    ...
    addCommsReply(tableSelectRandom(max_health_widget_prompts),function()
        ...
        addCommsReply(tableSelectRandom(get_max_health_diagnostic_prompt),function()
            if source:takeReputationPoints(5) then
                source.max_health_widgets = true
                target.max_health_widgets = false
                local max_health_installed_confirmation = {
                    "Installed",
                    string.format("%s has installed the max health diagnostic unit",target:getCallSign()),
                    "It's installed",
                    string.format("%s now has a max health diagnostic unit",source:getCallSign()),
                }
...
function presentShieldBanner()
    ...
    addCommsReply(tableSelectRandom(shield_diagnostic_prompts),function()
        local shield_diagnostic_explained = {
            ...
            string.format("There's a shield diagnostic unit here that could be installed on %s. Some engineers like the raw numbers it provides better than the standard percentage values. Do you want it installed for your engineer?",source:getCallSign()),
        ...
        addCommsReply(tableSelectRandom(install_shield_diagnostic_confirmation_prompt),function()
            if source:takeReputationPoints(5) then
                source.shield_banner = true
                target.shield_banner = false
                local shield_diagnostic_installed_confirmation = {
                    "Installed",
                    string.format("%s has installed the shield diagnostic unit",target:getCallSign()),
                    "It's installed",
                    string.format("%s now has a shield diagnostic unit",source:getCallSign()),
                }
...
function presentHullBanner()
    ...
    addCommsReply(tableSelectRandom(hull_diagnostic_prompts),function()
        local hull_diagnostic_explained = {
            ...
            string.format("There's a hull diagnostic unit here that could be installed on %s. Some engineers like the raw numbers it provides better than the standard percentage values. Do you want it installed for your engineer?",source:getCallSign()),
        ...
        addCommsReply(tableSelectRandom(install_hull_diagnostic_confirmation_prompt),function()
            if source:takeReputationPoints(5) then
                source.hull_banner = true
                target.hull_banner = false
                local hull_diagnostic_installed_confirmation = {
                    "Installed",
                    string.format("%s has installed the hull diagnostic unit",target:getCallSign()),
                    "It's installed",
                    string.format("%s now has a hull diagnostic unit",source:getCallSign()),
                }
...
function presentWaypointDistanceCalculator()
    ...
    addCommsReply(tableSelectRandom(waypoint_distance_calc_prompts),function()
        local waypoint_distance_calc_explained = {
            ...
            string.format("We have a waypoint distance calculator begging to be installed on %s. Helm or Tactical use it for extremely accurate distance calculations on waypoints placed by Relay or Operations. Would this be useful for you?",source:getCallSign()),
        }
        ...
        if source:hasPlayerAtPosition("Helms") then
            if source:hasPlayerAtPosition("Tactical") then
        ...
        elseif source:hasPlayerAtPosition("Tactical") then
        ...
        addCommsReply(tableSelectRandom(install_waypoint_distance_calc_confirmation_prompt),function()
            if source:takeReputationPoints(5) then
                source.way_dist = true
                target.way_dist = false
                local waypoint_distance_calc_installed_confirmation = {
                    "Installed",
                    string.format("%s has installed the waypoint distance calculator",target:getCallSign()),
                    "It's installed",
                    string.format("%s now has a waypoint distance calculator",source:getCallSign()),
                }
...
function presentBoostSensorRangeWithPower()
    ...
    addCommsReply(tableSelectRandom(boost_sensors_with_power_prompts),function()
        ...
        for i,sensor_booster in ipairs(target.installable_sensor_boost_ranges) do
            addCommsReply(string.format("Range interval:%sU Reputation:%s",sensor_booster.interval,sensor_booster.cost),function()
                if source:takeReputationPoints(sensor_booster.cost) then
                    source.power_sensor_interval = sensor_booster.interval
                    target.installable_sensor_boost_ranges[i] = target.installable_sensor_boost_ranges[#target.installable_sensor_boost_ranges]
                    target.installable_sensor_boost_ranges[#target.installable_sensor_boost_ranges] = nil
                    if #target.installable_sensor_boost_ranges == 0 then
                        target.installable_sensor_boost = false
                    end
                    local sensor_booster_installed_confirmation = {
                        "Installed",
                        string.format("%s has installed the sensor booster device",target:getCallSign()),
                        "It's installed",
                        string.format("%s now has a powered sensor booster",source:getCallSign()),
                    }
...
function presentReturnMaxHealthWidgets()
    local chunk_max_health_prompt = {
        ...
        string.format("Donate max health diagnostic unit to %s",target:getCallSign()),
        "Remove max health diagnostic unit. Give it to station",
        string.format("Transfer max health diagnostic unit from %s to station %s",source:getCallSign(),target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(chunk_max_health_prompt),function()
        local max_health_donation_confirmed = {
            string.format("%s thanks you and says they will put it to good use.",target:getCallSign()),
            string.format("Max health diagnostic unit uninstalled from %s. The technicians at %s say, 'Thanks %s. There are a number of other ships that have been asking for this.'",source:getCallSign(),target:getCallSign(),source:getCallSign()),
            string.format("%s thanks you for the donation of the max health diagnostic unit",target:getCallSign()),
            string.format("The max health diagnostic unit has been transferred from your ship to the parts inventory on station %s. They express their gratitude for your donation.",target:getCallSign()),
        }
        setCommsMessage(tableSelectRandom(max_health_donation_confirmed))
        source.max_health_widgets = false
        target.max_health_widgets = true
        target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(3,9))
		addCommsReply("Back to enhance ship",enhanceShip)
		addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
		addCommsReply(_("Back to station communication"), commsStation)
...
function presentReturnShieldBanner()
    local remove_shield_diagnostic_prompt = {
        ...
        string.format("Donate shield diagnostic unit to %s",target:getCallSign()),
        "Remove shield diagnostic unit. Give it to station",
        string.format("Transfer shield diagnostic unit from %s to station %s",source:getCallSign(),target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(remove_shield_diagnostic_prompt),function()
        local shield_diagnostic_donation_confirmed = {
            string.format("%s thanks you and says they will put it to good use.",target:getCallSign()),
            string.format("Shield diagnostic unit uninstalled from %s. The technicians at %s say, 'Thanks %s. There are a number of other ships that have been asking for this.'",source:getCallSign(),target:getCallSign(),source:getCallSign()),
            string.format("%s thanks you for the donation of the shield diagnostic unit",target:getCallSign()),
            string.format("The shield diagnostic unit has been transferred from your ship to the parts inventory on station %s. They express their gratitude for your donation.",target:getCallSign()),
        }
        setCommsMessage(tableSelectRandom(shield_diagnostic_donation_confirmed))
        source.shield_banner = false
        target.shield_banner = true
        target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(3,9))
		addCommsReply("Back to enhance ship",enhanceShip)
		addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
		addCommsReply(_("Back to station communication"), commsStation)
...
function presentReturnHullBanner()
    local remove_hull_diagnostic_prompt = {
        ...
        string.format("Donate hull diagnostic unit to %s",target:getCallSign()),
        "Remove hull diagnostic unit. Give it to station",
        string.format("Transfer hull diagnostic unit from %s to station %s",source:getCallSign(),target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(remove_hull_diagnostic_prompt),function()
        local hull_diagnostic_donation_confirmed = {
            string.format("%s thanks you and says they will put it to good use.",target:getCallSign()),
            string.format("Hull diagnostic unit uninstalled from %s. The technicians at %s say, 'Thanks %s. There are a number of other ships that have been asking for this.'",source:getCallSign(),target:getCallSign(),source:getCallSign()),
            string.format("%s thanks you for the donation of the hull diagnostic unit",target:getCallSign()),
            string.format("The hull diagnostic unit has been transferred from your ship to the parts inventory on station %s. They express their gratitude for your donation.",target:getCallSign()),
        }
        setCommsMessage(tableSelectRandom(hull_diagnostic_donation_confirmed))
        source.hull_banner = false
        target.hull_banner = true
        target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(3,9))
		addCommsReply("Back to enhance ship",enhanceShip)
		addCommsReply("Back to interactive relay officer",interactiveDockedStationCommsMeat)
		addCommsReply(_("Back to station communication"), commsStation)
...
function presentReturnWaypointDistanceCalculator()
    local remove_waypoint_dist_calc_prompt = {
        ...
        string.format("Donate waypoint distance calculator to %s",target:getCallSign()),
        "Remove waypoint distance calculator. Give it to station",
        string.format("Transfer waypoint distance calculator from %s to station %s",source:getCallSign(),target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(remove_waypoint_dist_calc_prompt),function()
        ...
        addCommsReply(tableSelectRandom(confirm_waypoint_dist_donation_prompt),function()
            source:addReputationPoints(5)
            source.way_dist = false
            target.way_dist = true
            target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(3,9))
            if source.way_distance_button_hlm ~= nil then
                source:removeCustom(source.way_distance_button_hlm)
                source:removeCustom(source.way_distance_button_tac)
                source.way_distance_button_hlm = nil
                source.way_distance_button_tac = nil
            end
            local confirm_uninstalled_waypoint_dist_calc = {
                ...
                string.format("The %s will go down in our records as a generous ship. We'll make sure another fleet member gets good use from your waypoint distance calculator",source:getCallSign()),
...
function minorUpgrades()
    ...
    if target.proximity_scanner == nil then
        if random(1,100) < 55 then
            target.proximity_scanner = true
            target.proximity_scanner_range = math.random(1,5)
        else
            target.proximity_scanner = false
        end
    end
    if target.max_health_widgets == nil then
    ...
    if target.shield_banner == nil then
    ...
    if target.hull_banner == nil then
        if random(1,100) < 50 then
            target.hull_banner = true
        else
            target.hull_banner = false
    ...
    if target.installable_sensor_boost == nil then
        if random(1,100) < 32 then
            target.installable_sensor_boost = true
            target.installable_sensor_boost_ranges = {}
            local sensor_boost_ranges_pool = {}
            for i=5,9,.5 do
                table.insert(sensor_boost_ranges_pool,{interval = i,cost=math.random(3,6)*2*i})
            end
            for i=1,3 do
                table.insert(target.installable_sensor_boost_ranges,tableRemoveRandom(sensor_boost_ranges_pool))
            end
        else
            target.installable_sensor_boost = false
        end
    ...
    if target.installable_sensor_boost and (source.power_sensor_interval == nil or source.power_sensor_interval == 0) then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 30 then
    ...
    if target.proximity_scanner and (source.prox_scan == nil or source.prox_scan < target.proximity_scanner_range) then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 50 then
    ...
    if target.max_health_widgets and not source.max_health_widgets then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 25 then
                table.insert(minor_upgrade_choices,presentMaxHealthWidgets)
            end
        elseif not target:isEnemy(source) then
            if target.comms_data.friendlyness > 45 then
                table.insert(minor_upgrade_choices,presentMaxHealthWidgets)
            end
        end
    end
    if target.shield_banner and not source.shield_banner then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 20 then
    ...
    if target.hull_banner and not source.hull_banner then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 30 then
    ...
    if target.way_dist and not source.way_dist then
        if target:isFriendly(source) then
            if target.comms_data.friendlyness > 10 then
    ...
    if not target.max_health_widgets and source.max_health_widgets ~= nil and source.max_health_widgets then
        table.insert(return_minor_upgrade_choices,presentReturnMaxHealthWidgets)
    end
    if not target.shield_banner and source.shield_banner ~= nil and source.shield_banner then
        table.insert(return_minor_upgrade_choices,presentReturnShieldBanner)
    end
    if not target.hull_banner and source.hull_banner ~= nil and source.hull_banner then
        table.insert(return_minor_upgrade_choices,presentReturnHullBanner)
    end
    if not target.way_dist and source.way_dist ~= nil and source.way_dist then
    ...
    if #minor_upgrade_choices + #return_minor_upgrade_choices > 0 then
        ...
        local minor_upgrade_prompt = {
            "Minor upgrade",
            "Get a minor upgrade",
            string.format("Minor upgrade for %s",source:getCallSign()),
            string.format("Check minor upgrades on %s",target:getCallSign()),
        }
        addCommsReply(tableSelectRandom(minor_upgrade_prompt),function()
            local minor_upgrades_available = {
                ...
                string.format("Here are some minor upgrades available here on %s. Let me know if any of these seem interesting.",target:getCallSign()),
            }
...
function overchargeShipSystems()
    ...
    local max_charge = source.max_jump_range
    if max_charge == nil then
        max_charge = 50000
    end
    if target.comms_data.jump_overcharge and source:hasJumpDrive() then
        if source:getJumpDriveCharge() >= max_charge then
            if target:isFriendly(source) then
                if target.comms_data.friendlyness > 20 then
    ...
    if target.comms_data.shield_overcharge and source:getShieldCount() > 0 then
        if source:getShieldLevel(0) == source:getShieldMax(0) then
            if target:isFriendly(source) then
                if target.comms_data.friendlyness > 25 then
                    overcharge_available = true
        ...
        if source:getShieldCount() > 1 and source:getShieldLevel(1) == source:getShieldMax(1) then
            if target:isFriendly(source) then
                if target.comms_data.friendlyness > 30 then
                    overcharge_available = true
    ...
    if overcharge_available then
        ...
        local overcharge_system_prompt = {
            "Overcharge system",
            "Overcharge a system",
            string.format("Overcharge a system on %s",source:getCallSign()),
        ...
        addCommsReply(tableSelectRandom(overcharge_system_prompt),function()
            ...
            if jump_overcharge_available then
                ...
                addCommsReply(tableSelectRandom(overcharge_jump_prompt),function()
                    if source:takeReputationPoints(overcharge_cost) then
                        source:setJumpDriveCharge(source:getJumpDriveCharge() + max_charge)
                        local jump_drive_overcharged = {
                            string.format("Your jump drive has been overcharged to %ik",math.floor(source:getJumpDriveCharge()/1000)),
                            string.format("%s's jump drive has been overcharged to %ik",source:getCallSign(),math.floor(source:getJumpDriveCharge()/1000)),
                            string.format("We have overcharged your jump drive to %ik",math.floor(source:getJumpDriveCharge()/1000)),
                            string.format("%s's jump drive now has total charge of %ik",source:getCallSign(),math.floor(source:getJumpDriveCharge()/1000)),
                        }
            ...
            if front_shield_overcharge_available then
                ...
                if target:isFriendly(source) then
                    ...
                    elseif target.comms_data.friendlyness > 70 then
                        base_front_shield_overcharge_cost = 7
                    end
                elseif not target:isEnemy(source) then
                    if target.comms_data.friendlyness > 90 then
                        base_front_shield_overcharge_cost = 5
                    elseif target.comms_data.friendlyness > 75 then
                        base_front_shield_overcharge_cost = 7
                    end
                end
                if source:getReputationPoints() > 2*base_front_shield_overcharge_cost then
                    ...
                    addCommsReply(tableSelectRandom(front_shield_overcharge_range_prompt),function()
                        ...
                        for i=1,4 do
                            if i*base_front_shield_overcharge_cost <= source:getReputationPoints() then
                                ...
                                addCommsReply(tableSelectRandom(overcharge_amount_prompts),function()
                                    if source:takeReputationPoints(i*base_front_shield_overcharge_cost) then
                                        if source:getShieldCount() == 1 then
                                            source:setShields(source:getShieldMax(0)*(1 + i*5/100))
                                        else
                                            source:setShields(source:getShieldMax(0)*(1 + i*5/100),source:getShieldLevel(1))
                                        end
                                        local overcharge_front_shield_confirmation = {
                                            "Your front shield has been overcharged",
                                            "Overcharge applied to front shield",
                                            string.format("%s's front shield has now been overcharged",source:getCallSign()),
                    ...
                    addCommsReply(tableSelectRandom(front_shield_overcharge_single_prompt),function()
                        if source:takeReputationPoints(base_front_shield_overcharge_cost) then
                            if source:getShieldCount() == 1 then
                                source:setShields(source:getShieldMax(0)*1.05)
                            else
                                source:setShields(source:getShieldMax(0)*1.05,source:getShieldLevel(1))
                            end
                            local overcharge_front_shield_confirmation = {
                                "Your front shield has been overcharged",
                                "Overcharge applied to front shield",
            ...
            if rear_shield_overcharge_available then
                ...
                if source:getReputationPoints() > 2*base_rear_shield_overcharge_cost then
                    ...
                    addCommsReply(tableSelectRandom(rear_shield_overcharge_range_prompt),function()
                        ...
                        for i=1,4 do
                            if i*base_rear_shield_overcharge_cost <= source:getReputationPoints() then
                                ...
                                addCommsReply(tableSelectRandom(overcharge_rear_amount_prompts),function()
                                    if source:takeReputationPoints(i*base_rear_shield_overcharge_cost) then
                                        source:setShields(source:getShieldLevel(0),source:getShieldMax(1)*(1 + i*5/100))
                                        local overcharge_rear_shield_confirmation = {
                                            "Your rear shield has been overcharged",
                                            "Overcharge applied to rear shield",
                                            string.format("%s's rear shield has now been overcharged",source:getCallSign()),
                    ...
                    addCommsReply(tableSelectRandom(rear_shield_overcharge_single_prompt),function()
                        if source:takeReputationPoints(base_rear_shield_overcharge_cost) then
                            source:setShields(source:getShieldLevel(0),source:getShieldMax(1)*1.05)
                            local overcharge_rear_shield_confirmation = {
                                "Your rear shield has been overcharged",
                                "Overcharge applied to rear shield",
                                string.format("%s's rear shield has now been overcharged",source:getCallSign()),
...
function boostSensorsWhileDocked()
    local temporary_station_sensor_boost_prompt = {
        string.format("Augment scan range with station sensors while docked (%i rep)",target.comms_data.sensor_boost.cost),
        string.format("Increase sensor range via station sensors while docked (%i rep)",target.comms_data.sensor_boost.cost),
        string.format("Tie in to station sensors to boost range while docked (%i rep)",target.comms_data.sensor_boost.cost),
        string.format("Connect to station sensors in docking bay to boost range (%i rep)",target.comms_data.sensor_boost.cost),
    }
    addCommsReply(tableSelectRandom(temporary_station_sensor_boost_prompt),function()
        if source:takeReputationPoints(target.comms_data.sensor_boost.cost) then
            if source.normal_long_range_radar == nil then
                source.normal_long_range_radar = source:getLongRangeRadarRange()
            end
            source.station_sensor_boost = target.comms_data.sensor_boost.value
            local confirm_sensor_boost = {
                string.format("Sensors increased by %i units",math.floor(target.comms_data.sensor_boost.value/1000)),
                string.format("Sensor range increased by %i units",math.floor(target.comms_data.sensor_boost.value/1000)),
                string.format("In conjunction with %s's sensors, our sensor range has been increased by %i units",target:getCallSign(),math.floor(target.comms_data.sensor_boost.value/1000)),
                string.format("%s's and %s's sensors in tandem have increased our sensor range by %i units",source:getCallSign(),target:getCallSign(),math.floor(target.comms_data.sensor_boost.value/1000)),
            }
...
function increaseSecurityMorale()
    ...
    addCommsReply(tableSelectRandom(select_training_prompt),function()
        local security_morale_training_explained = {
            ...
            string.format("There are several security officer training courses in %s's library. They're designed to boost morale and competence. Pick one for 5 reputation to run in your simulation systems if you like.",target:getCallSign()),
        }
            ...
            addCommsReply(title,function()
                if source:takeReputationPoints(5) then
                    local got_the_training_module = {
                        ...
                        string.format("%s has transmitted '%s' to your simulation systems. Our reading show that some of your security team is already using it",target:getCallSign(),title),
                        string.format("%s's simulation systems have downloaded '%s.' At least one of your security officers has already started running it.",source:getCallSign(),title),
...
function getSpecialtyProbes()
    local get_specialty_probes_prompt = {
        ...
        string.format("Get specialty probes from %s",target:getCallSign()),
    }
    addCommsReply(tableSelectRandom(get_specialty_probes_prompt),function()
        local specialty_probes = {}
        if target.comms_data.fast_probes ~= nil and target.comms_data.fast_probes.quantity > 0 then
            specialty_probes["fast_probes"] = {
                quantity = target.comms_data.fast_probes.quantity,            
                singular_desc = "One batch of %s type probes",                
                plural_desc = "%i batches of %s type probes",            
                name = target.comms_data.fast_probes.name,            
                cost = target.comms_data.fast_probes.cost,
                increment = 5,
                prompt = "Purchase five %s type probes for %i reputation",
                response = "Five %s type probes have been added",
            }
        end
        if target.comms_data.remote_warp_jammer ~= nil and target.comms_data.remote_warp_jammer.quantity > 0 then
            specialty_probes["remote_warp_jammer"] = {
                quantity = target.comms_data.remote_warp_jammer.quantity,    
                singular_desc = "One remote warp jammer kit of type %s",    
                plural_desc = "%i remote warp jammer kits of type %s",    
                name = target.comms_data.remote_warp_jammer.name,        
                cost = target.comms_data.remote_warp_jammer.cost,        
                increment = 1,
                prompt = "Purchase %s type probe for %i reputation",
                response = "A %s type probe has been added",
            }
        end
        if target.comms_data.sensor_boost_probes ~= nil and target.comms_data.sensor_boost_probes.quantity > 0 then
            specialty_probes["sensor_boost_probes"] = {
                quantity = target.comms_data.sensor_boost_probes.quantity,    
                singular_desc = "One sensor boost probe of type %s",        
                plural_desc = "%i sensor boost probes of type %s",        
                name = target.comms_data.sensor_boost_probes.name,    
                cost = target.comms_data.sensor_boost_probes.cost,    
                increment = 1,
                prompt = "Purchase %s type probe for %s reputation",
                response = "A %s type probe has been added",
            }
        end
        if target.comms_data.mine_probes ~= nil and target.comms_data.mine_probes.quantity > 0 then
            specialty_probes["mine_probes"] = {
                quantity = target.comms_data.mine_probes.quantity,            
                singular_desc = "One mine probe of type %s",                
                plural_desc = "%i mine probes of type %s",                
                name = target.comms_data.mine_probes.name,            
                cost = target.comms_data.mine_probes.cost,            
                increment = 1,
                prompt = "Purchase %s type probe for %s reputation",
                response = "A %s type probe has been added",
            }
        ...
        for specialty_probe,sp in pairs(specialty_probes) do
            if sp.quantity ~= nil and sp.quantity > 0 then
                ...
                addCommsReply(string.format(sp.prompt,sp.name,sp.cost),function()
                    if sp == "mine_probes" and source:getWeaponStorageMax("Mine") < target.comms_data.mine_probes.mines_required then
                        setCommsMessage(string.format("This mine probe kit requires %i mines. Your ship specification maxes out at %i mines. Upgrade your ship to store more mines and come back.",target.comms_data.mine_probes.mines_required,source:getWeaponStorageMax("Mine")))
                    else
                        if source:takeReputationPoints(sp.cost) then
                            target.comms_data[specialty_probe].quantity = target.comms_data[specialty_probe].quantity - 1
                            if source.probe_type_list == nil then
                                source.probe_type_list = {}
                                table.insert(source.probe_type_list,{name = "standard", count = -1})
                            end
                            local matching_index = 0
                            for probe_type_index, probe_type_item in ipairs(source.probe_type_list) do
                            ...
                            if matching_index > 0 then
                                source.probe_type_list[matching_index].count = source.probe_type_list[matching_index].count + sp.increment
                            else
                                --    add variants for different probe types
                                if specialty_probe == "remote_warp_jammer" then
                                    table.insert(source.probe_type_list,{name = sp.name, count = sp.increment, speed = target.comms_data.remote_warp_jammer.speed, warp_jam_range = target.comms_data.remote_warp_jammer.warp_jam_range})
                                elseif specialty_probe == "sensor_boost_probes" then
                                    table.insert(source.probe_type_list,{name = sp.name, count = sp.increment, speed = target.comms_data.sensor_boost_probes.speed, boost = target.comms_data.sensor_boost_probes.boost, range = target.comms_data.sensor_boost_probes.range})
                                elseif specialty_probe == "mine_probes" then
                                    table.insert(source.probe_type_list,{name = sp.name, count = sp.increment, speed = target.comms_data.mine_probes.speed, mine_fetus = target.comms_data.mine_probes.mine_fetus, mines_required = target.comms_data.mine_probes.mines_required})
                                else    --fast probes
                                    table.insert(source.probe_type_list,{name = sp.name, count = sp.increment, speed = target.comms_data.fast_probes.speed})
                                end
                            end
                            setCommsMessage(string.format(sp.response,sp.name))
                            if source.probe_type == nil then
                                source.probe_type = "standard"
                            end
                            if source.probe_type_button == nil then
                                source.probe_type_button = "probe_type_button"
                                source:addCustomButton("Relay",source.probe_type_button,"Probes: standard",function()
                                    string.format("")
                                    cycleProbeType(source)
                                end,10)
                            end
                            if source.probe_type_button_ops == nil then
                                source.probe_type_button_ops = "probe_type_button_ops"
                                source:addCustomButton("Operations",source.probe_type_button_ops,"Probes: standard",function()
                                    string.format("")
                                    cycleProbeType(source)
                                end,10)
                            end
        ...
        addCommsReply("What exactly are specialty probes?",function()
            ...
            if target.comms_data.fast_probes ~= nil and target.comms_data.fast_probes.quantity > 0 then
                kit_description = string.format("%s The %s kits come in batches of 5. You attach a kit to one of the probes you launch. The probe then travels at %.1f units per minute to reach its destination. Normal probes travel at 60 units per minute.",kit_description,target.comms_data.fast_probes.name,target.comms_data.fast_probes.speed*60/1000)
            end
            if target.comms_data.remote_warp_jammer ~= nil and target.comms_data.remote_warp_jammer.quantity > 0 then
                kit_description = string.format("%s The %s kits are purchased singly. When attached, the probe travels at %.1f units per minute to reach its destination. Once the probe arrives, it drops a warp jammer with a jamming radius of %i units.",kit_description,target.comms_data.remote_warp_jammer.name,target.comms_data.remote_warp_jammer.speed*60/1000,math.floor(target.comms_data.remote_warp_jammer.warp_jam_range/1000))
            end
            if target.comms_data.sensor_boost_probes ~= nil and target.comms_data.sensor_boost_probes.quantity > 0 then
                kit_description = string.format("%s The %s kits are purchased individually. When attached, the probe enhances sensors by %i units if within %i units. The sensor boost gradually decreases until there is no boost at %i units distance from the probe.",kit_description,target.comms_data.sensor_boost_probes.name,target.comms_data.sensor_boost_probes.boost,target.comms_data.sensor_boost_probes.range/2,target.comms_data.sensor_boost_probes.range)
            end
            if target.comms_data.mine_probes ~= nil and target.comms_data.mine_probes.quantity > 0 then
                kit_description = string.format("%s The %s kits are purchased individually. When attached along with a mine or mines from supply, the probe carries the mine(s) inert to the probe destination. Once it reaches its destination, it enters a 5 second preparation phase. After that, it enters a stealth mode for 5 - 15 seconds. After the stealth mode, it primes the mine(s).",kit_description,target.comms_data.mine_probes.name)
            end
...
function unloadEscapePods(cost)
    if cost == nil then
        local unload_escape_pod_prompt = {
            ...
            string.format("Offload the escape pods currently on %s",source:getCallSign()),
            string.format("Release the scape pods into %s's care",target:getCallSign()),
            string.format("Unload the escape pods that %s retrieved earlier",source:getCallSign()),
        }
        addCommsReply(tableSelectRandom(unload_escape_pod_prompt),function()
            local pods_aboard = source.max_pods - source.pods
            source.pods = source.max_pods
            if pods_aboard > 1 then
                local plural_escape_pod_response = {
                    string.format("Escape pods unloaded and placed in the care of station %s. %s may now retrieve up to %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                    string.format("%s is now taking care of the escape pods retrieved by %s. With the space freed up, we may now retrieve up to %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                    string.format("[%s docking bay supervisor]\n'All escape pods unloaded, %s. We'll take care of them from here.'\nUnloading those escape pods frees us up to pick up as many as %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                    string.format("%s has secured all of %s's escape pods. %i open escape pod slots now available.",target:getCallSign(),source:getCallSign(),source.pods),
                }
                setCommsMessage(tableSelectRandom(plural_escape_pod_response))
            else
                if source.max_pods > 1 then
                    local single_unload_plural_available_pods = {
                        string.format("Escape pod unloaded into the care of %s. %s may now retrieve up to %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                        string.format("%s is now taking care of the escape pod retrieved by %s. With the space freed up, we may now retrieve up to %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                        string.format("[%s docking bay supervisor]\n'Your escape pod has been unloaded, %s. We'll take care of it from here.'\nUnloading those escape pods frees us up to pick up as many as %i escape pods",target:getCallSign(),source:getCallSign(),source.pods),
                        string.format("%s has secured the escape pod from %s. %i open escape pod slots are now available",target:getCallSign(),source:getCallSign(),source.pods),
                    }
                    setCommsMessage(tableSelectRandom(single_unload_plural_available_pods))
                else
                    local single_unload_and_available_pods = {
                        string.format("Escape pod unloaded and placed in the care of station %s. %s may now retrieve one escape pod",target:getCallSign(),source:getCallSign()),
                        string.format("%s is now taking care of the escape pod retrieved by %s. We may now retrieve another escape pod.",target:getCallSign(),source:getCallSign()),
                        string.format("[%s docking bay supervisor]\n'We unloaded your escape pod, %s. We'll take care of it from here.'\nWe may now retrieve another escape pod.",target:getCallSign(),source:getCallSign()),
                        string.format("%s has secured the escape pod from %s. You may retrieve another escape pod if you wish.",target:getCallSign(),source:getCallSign()),
                    }
        ...
        local unload_pods_at_cost_prompt = {
            ...
            string.format("Unload pods to %s (%i rep per pod)",target:getCallSign(),cost),
            string.format("Unload escape pods (%i reputation per pod)",cost),
            string.format("Unload retrieved escape pods (%i reputation)",cost*(source.max_pods - source.pods)),
        }
        addCommsReply(tableSelectRandom(unload_pods_at_cost_prompt),function()
            if source:takeReputationPoints(cost*(source.max_pods - source.pods)) then
                local pods_aboard = source.max_pods - source.pods
                source.pods = source.max_pods
                if pods_aboard > 1 then
                    local plural_escape_pod_response = {
...
function goodsCommerce()
    ...
    local trade_goods = ""
    local player_goods = ""
    if target.comms_data.goods ~= nil then
        for good, good_data in pairs(target.comms_data.goods) do
            if good_data.quantity ~= nil and good_data.quantity > 0 then
                station_sells = true
    ...
    if target.comms_data.trade ~= nil then
        if station_sells then
            for good,trade_bool in pairs(target.comms_data.trade) do
                if trade_bool then
                    station_trades = true
                    if trade_goods == "" then
    ...
    if commerce_available then
        local commerce_out = ""
        if station_sells then
            commerce_out = string.format(_("trade-comms","%s sells %s."),target:getCallSign(),goods_for_sale)
			local buy_goods_prompts = {
				"Buy goods",
				"Purchase goods",
				string.format("Buy goods from %s",target:getCallSign()),
				string.format("Purchase goods from %s",target:getCallSign()),
			}
			addCommsReply(tableSelectRandom(buy_goods_prompts),buyGoodsFromStation)
        end
        if station_buys then
            if commerce_out == "" then
                commerce_out = string.format(_("trade-comms","%s buys %s."),target:getCallSign(),will_buy_goods)
            else
                commerce_out = string.format(_("trade-comms","%s\n%s buys %s."),commerce_out,target:getCallSign(),will_buy_goods)
            end
            local buy_match = false
            if player_has_goods then
                for buy_good, price in pairs(target.comms_data.buy) do
                    for good, good_quantity in pairs(source.goods) do
            ...
            if buy_match then
				local sell_goods_prompts = {
					...
					string.format("Sell goods to %s",target:getCallSign()),
					string.format("Sell goods to %s for reputation",target:getCallSign()),
				}
        ...
        if station_trades then
            if commerce_out == "" then
                commerce_out = string.format(_("trade-comms","%s trades %s for %s."),target:getCallSign(),goods_for_sale,trade_goods)
            else
                commerce_out = string.format(_("trade-comms","%s\n%s trades %s for %s."),commerce_out,target:getCallSign(),goods_for_sale,trade_goods)
            end
            local trade_match = false
            if player_has_goods then
                for trade_good,trade_bool in pairs(target.comms_data.trade) do
                    for good, good_quantity in pairs(source.goods) do
            ...
            if trade_match then
				local trade_goods_prompts = {
					...
					"Exchange goods",
					"Barter goods",
					string.format("Trade goods with %s",target:getCallSign()),
				}
        ...
        if player_has_goods then
            if commerce_out == "" then
                commerce_out = string.format(_("trade-comms","%s has %s aboard."),source:getCallSign(),player_goods)
            else
                commerce_out = string.format(_("trade-comms","%s\n%s has %s aboard."),commerce_out,source:getCallSign(),player_goods)
            end
			...
			local donate_goods_prompts = {
				...
				string.format("Give goods to %s",target:getCallSign()),
				string.format("Donate goods to %s",target:getCallSign()),
			}
        ...
        local no_commerce_options = {
            ...
            string.format("No commercial options available at %s",target:getCallSign()),
            string.format("%s has no available commercial options",target:getCallSign()),
        }
...
function tradeGoodsWithStation()
    ...
    if target.comms_data.trade ~= nil then
        for trade_good, trade_bool in pairs(target.comms_data.trade) do
            if source.goods ~= nil then
                if source.goods[trade_good] ~= nil then
                    if source.goods[trade_good] > 0 then
                        for good, good_data in pairs(target.comms_data.goods) do
                                ...
                                addCommsReply(string.format(_("trade-comms","Trade %s for %s"),good_desc[trade_good],good_desc[good]),function()
                                    if not source:isDocked(target) then
                                        local stay_docked_to_trade = {
                                            ...
                                            string.format("You must stay docked long enough for a trade between %s and %s to be completed.",source:getCallSign(),target:getCallSign()),
                                            string.format("You undocked before %s could complete the trade you wanted.",target:getCallSign()),
                                        }
                                    ...
                                    if good_data.quantity < 1 then
                                        local insufficient_station_inventory = {
                                            ...
                                            string.format("%s does not have enough inventory",target:getCallSign()),
                                            string.format("Not enough inventory on %s",target:getCallSign()),
                                        }
                                        setCommsMessage(tableSelectRandom(insufficient_station_inventory))
                                    else
                                        good_data.quantity = good_data.quantity - 1
                                        if source.goods[good] == nil then
                                            source.goods[good] = 0
                                        end
                                        source.goods[good] = source.goods[good] + 1
                                        source.goods[trade_good] = source.goods[trade_good] - 1
                                        local trade_confirmation = {
                                            ...
                                            string.format("%s agreed to trade a %s for a %s",target:getCallSign(),good_desc[trade_good],good_desc[good]),
                                        ...
                                        target.comms_data.friendlyness = math.min(100,target.comms_data.friendlyness + random(2,5))
                                    end
...
function sellGoodsToStation()
    ...
    if target.comms_data.buy ~= nil then
        for good, price in pairs(target.comms_data.buy) do
            if source.goods[good] ~= nil and source.goods[good] > 0 then
                ...
                addCommsReply(tableSelectRandom(sell_a_good_prompt), function()
                    if not source:isDocked(target) then
                        local stay_docked_to_sell = {
                            ...
                            string.format("You must stay docked long enough for a sale between %s and %s to be completed.",source:getCallSign(),target:getCallSign()),
                            string.format("You undocked before %s could complete the sale you requested.",target:getCallSign()),
                        }
                    ...
                    local sale_results = {
                        "One sold",
                        "You sold one",
                        string.format("You sold one to %s",target:getCallSign()),
                        string.format("%s bought one from you",target:getCallSign()),
                    }
                    ...
                    source.goods[good] = source.goods[good] - 1
                    source:addReputationPoints(price)
                    source.cargo = source.cargo + 1
        ...
        local no_goods_to_buy = {
            ...
            string.format("%s is no longer in the market to buy goods",target:getCallSign()),
            string.format("%s has left the goods buying market",target:getCallSign()),
            string.format("%s no longer wants to buy any goods",target:getCallSign()),
        }
    ...
    if good_match_count == 0 then
        local no_matching_sellable_goods = {
            ...
            string.format("You have nothing %s is interested in.",target:getCallSign()),
            string.format("%s is not interested in any goods you have.",target:getCallSign()),
            string.format("[%s purchasing agent]\n'Sorry, %s. You have nothing that interests us.'\nYou hear the sound of a ledger book closing just before the mic cuts off.",target:getCallSign(),source:getCallSign()),
        }
    ...
    if target.comms_data.goods ~= nil then
        for good, good_data in pairs(target.comms_data.goods) do
            local buy_goods_at_price_prompts = {
                ...
                string.format("Buy %s from %s for %i rep",good_desc[good],target:getCallSign(),good_data["cost"]),
            ...
            addCommsReply(tableSelectRandom(buy_goods_at_price_prompts), function()
                if not source:isDocked(target) then
                    local stay_docked_to_buy = {
                        ...
                        string.format("You must stay docked long enough for a sale between %s and %s to be completed.",target:getCallSign(),source:getCallSign()),
                        string.format("You undocked before %s could complete the sale you requested.",target:getCallSign()),
                    }
                ...
                local quantity_of_good = {
                    ...
                    string.format("%s's quantity: %s",target:getCallSign(),good_data["quantity"]),
                    string.format("Quantity on hand: %s",good_data["quantity"]),
                }
                local purchase_results = {
                    "One bought",
                    "You bought one",
                    string.format("You purchased one from %s",target:getCallSign()),
                    string.format("%s sold one to you",target:getCallSign()),
                }
                ...
                if source.cargo < 1 then
                    local insufficient_cargo_space_addendum = {
                        "Insufficient cargo space for purchase",
                        "You don't have enough room in your cargo hold",
                        string.format("Insufficient room in %s's cargo hold",source:getCallSign()),
                        string.format("%s does not have enough available cargo space",source:getCallSign()),
                    }
                    goodTransactionMessage = string.format("%s\n%s",goodTransactionMessage,tableSelectRandom(insufficient_cargo_space_addendum))
                elseif good_data["cost"] > math.floor(source:getReputationPoints()) then
                    local insufficient_rep_responses = {
                        "Insufficient reputation",
                        "Not enough reputation",
                        "You need more reputation",
                ...
                elseif good_data["quantity"] < 1 then
                    ...
                    if source:takeReputationPoints(good_data["cost"]) then
                        source.cargo = source.cargo - 1
                        good_data["quantity"] = good_data["quantity"] - 1
                        if source.goods == nil then
                            source.goods = {}
...
function jumpCorridor()
    ...
    for name, jc_item in pairs(jump_corridor_list) do
        if target == jc_item.station then
    ...
    if skeleton_docked then
        ...
        if all_docked then
            origin_jc_item = nil
            for name, jc_item in pairs(jump_corridor_list) do
                if string.find(name,target:getCallSign()) then
            ...
            addCommsReply(tableSelectRandom(take_jump_corridor_prompts),function()
                local choose_jump_station = {
                    ...
                    string.format("[%s jump corridor operator]\n'What is your station destination, %s?'\nThe jump corridor operator sounds like a bright and happy sentient being.",target:getCallSign(),source:getCallSign()),
                }
                ...
                for name, jc_item in pairs(jump_corridor_list) do
                    if target ~= jc_item.station then
                        addCommsReply(string.format("Take jump corridor to %s",jc_item.station:getCallSign()),function()
                            ...
                            local jt = target:getObjectsInRange(5000)
                            jump_train = {}
                            if #jt > 0 then
                                for index, ship in ipairs(jt) do
                                    if ship:isValid() and ship.typeName == "CpuShip" and ship:isDocked(target) then
...
function isAllowedTo(state)
    if state == "friend" and source:isFriendly(target) then
        return true
    end
    if state == "neutral" and not source:isEnemy(target) then
        return true
    end
    return false
end
function getWeaponCost(weapon)
    if comms_data.weapon_cost == nil then
        print("comms data weapons cost is nil. Station:",target:getCallSign())
    end
    if comms_data.weapon_cost[weapon] == nil then
        print("comms data weapon cost for weapon",weapon,"is nil. Station:",target:getCallSign())
    end
    return math.ceil(comms_data.weapon_cost[weapon] * comms_data.reputation_cost_multipliers[getFriendStatus()])
end
function getServiceCost(service)
    if comms_data.service_cost == nil then
        print("comms data service cost is nil. Station:",target:getCallSign())
        return 9999
    elseif comms_data.service_cost[service] == nil then
        print("comms data service cost for service",service,"is nil. Station:",target:getCallSign())
        return 9999
    else
...
function intimidateStationComms(source, target)
	if (source.special_intimidate_stations or source:getResourceAmount("Station Boarding Pod") > 0) and target:getFaction() ~= "Human Navy" then
		local current_faction = target:getFaction()
		setCommsMessage(_("special-comms", "You are our declared enemy. What do you want?"))
		local cost = special_buy_cost(target, source)
		addCommsReply(string.format(_("special-comms", "Surrender now! [Cost: %s Rep.]"), cost), function()

			if not target:areEnemiesInRange(5000) then
				setCommsMessage(_("needRep-comms", "We will not surrender unless threatened."))
			elseif not (target:getHull() < target:getHullMax()) then
				setCommsMessage(_("needRep-comms", "We will not surrender until our hull is damaged."))
			else
				target:setFaction("Human Navy")
				if target:areEnemiesInRange(5000) then
					target:setFaction(current_faction)
					setCommsMessage(_("needRep-comms", "We will not surrender as long as enemies of the Human Navy are still near."))
				elseif not source:takeReputationPoints(cost) then
					target:setFaction(current_faction)
					setCommsMessage(_("needRep-comms", "Insufficient reputation"))
				else
					target:setFaction("Independent")
...
function buyStationComms(source, target)
	if (source.special_buy_stations or source:getResourceAmount("Station Command Team") > 0) and target:getFaction() ~= "Human Navy" then
		local cost = special_buy_cost(target, source)
		addCommsReply(string.format(_("special-comms", "Submit your station to the Human Navy. [Cost: %s Rep.]"), cost), function()
			if not source:takeReputationPoints(cost) then
				setCommsMessage(_("needRep-comms", "Insufficient reputation"))
			else
				target:setFaction(source:getFaction())
				local gain = target:getHullMax() * 4 / 60
...
-- `source` and `target` are global in comms script.
--commsStation(source, target)
