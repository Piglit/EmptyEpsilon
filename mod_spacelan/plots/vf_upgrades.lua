-- manages player ship upgraded
-- originally written for shattered horizon
-- internal table keys are callsigns; this will be an issue if two ships with the same name appear.
...
function vf_upgrades:onNewPlayerShip(ship)
...
function vf_upgrades:get_player_ship_upgrade_level(ship, upgrade)
	local upgrade_levels = self.upgrades_by_ship[ship:getCallSign()]
	if upgrade_levels == nil then
		return 0
	end
	local level = upgrade_levels[upgrade]
	if level == nil then
		return 0
	end
	return level
...
function vf_upgrades:updatePlayerShip(delta, ship)
	if ship.upgrade_changed then
		local upgrade_levels = self.upgrades_by_ship[ship:getCallSign()]
		if upgrade_levels ~= nil then
...
function vf_upgrades:upgrades_gm_menu(shipname)
	clearGMFunctions()
	addGMFunction(_("buttonGM", "Upgrades for ") .. shipname, function()
		vf_upgrades:upgrades_gm_menu(shipname)
	end, 0)		--just an info
	local current_upgrades = vf_upgrades.upgrades_by_ship[shipname]
	if current_upgrades == nil then
		current_upgrades = {}
	end
	for up_idx, up_name in ipairs(vf_upgrades.possible_upgrades) do
		...
		addGMFunction(_("buttonGM", up_name) .. " " .. level, function()
			...
			for i=0,2 do	
				addGMFunction(tostring(i), function() 
					vf_upgrades:set_player_ship_upgrade_level(shipname, up_name, i)
					vf_upgrades:upgrades_gm_menu(shipname)
				end, i+1)
			end
			gm_menu_back()
...
function vf_upgrades:set_player_ship_upgrade_level(shipname, up_name, level)
	if type(shipname) == "table" then
		shipname.upgrade_changed = true	-- FIXME: not set when GM changes it via shipname string
		shipname = shipname:getCallSign()
	end
...
function vf_upgrades:upgrades_engineering_menu(ship, upgrade_levels)
		...
		if level == nil or level == 0 then
			...
			if ship.upgrades_active[up_name] ~= nil then
				...
				customElements:addCustomButton(ship, "Engineering", "deactivate_"..up_name, up_name .. _("-Boost stoppen"), function()
					vf_upgrades:deactivate_upgrade(ship, up_name)
				...
				if level > 0 then
					...
					customElements:addCustomButton(ship, "Engineering", "activate_"..up_name, up_name .. _("-Boost starten"), function()
						vf_upgrades:activate_upgrade(ship, up_name, level)
...
function vf_upgrades:activate_upgrade(ship, up_name, level)
	log("activate_upgrade", ship:getCallSign(), up_name, level)
	if level < 1 then
		vf_upgrades:deactivate_upgrade(ship, up_name)
	end
	if level > 2 then
...
function vf_upgrades:deactivate_upgrade(ship, up_name)
	log("deactivate_upgrade", ship:getCallSign(), up_name)
	if up_name == "Laser" then
		for idx=0,7 do
			ship:setBeamWeapon(idx,
						 ship.beam_weapon_data[idx].arc,
