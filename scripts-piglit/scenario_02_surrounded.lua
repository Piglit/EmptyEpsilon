...
function init()
    ...
    for _ = 1, 5 do
        local ship = CpuShip():setTemplate("Phobos T3"):orderRoaming():setFaction(enemy_faction)
    ...
    for _ = 1, 2 do
        local ship = CpuShip():setTemplate("Piranha F12"):orderRoaming():setFaction(enemy_faction)
        table.insert(enemyList, ship)
        setCirclePos(ship, random(0, 360), random(7000, 10000))
    end
    ...
    do
        ...
        local d = 9000
        local ship = CpuShip():setTemplate("Atlantis X23"):setRotation(a + 180):orderRoaming():setFaction(enemy_faction)
        table.insert(enemyList, ship)
