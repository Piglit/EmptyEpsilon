...
: GuiElement(owner, id), load_type(MW_None), manual_aim(false), missile_target_angle(0)
{
    ...
    load_type_rows[MW_IonMissile].button->setIcon("gui/icons/weapon-emp.png");
...
void GuiMissileTubeControls::onUpdate()
{
    ...
    for (int n = 0; n < my_spaceship->weapon_tube_count; n++)
    {
        ...
        else if (tube.canOnlyLoad(MW_LaserGreen) || tube.canOnlyLoad(MW_LaserRed) || tube.canOnlyLoad(MW_IonMissile))
        {
    ...
    if (keys.weapons_select_ion.getDown())
        selectMissileWeapon(MW_IonMissile);
