...
IonMissile::IonMissile(): MissileWeapon("IonMissile", MissileWeaponData::getDataFor(MW_IonMissile))
{
    setRadarSignatureInfo(0.0, 0.8, 0.0);
    setCollisionBox({10, 30}); // Make it a bit harder to the HVLI to phase trough smaller enemies
    radar_sprite = "radar/laser_bullet.png";
...
void IonMissile::particleEffect()
{
    ParticleEngine::spawn(glm::vec3(getPosition().x, getPosition().y, -10), glm::vec3(getPosition().x, getPosition().y, -10), glm::vec3(0, 0, 0.8), glm::vec3(0, 0, 0), 5, 5, 1.0);
}
