...
def make_scripts_locale(rootdir):
    os.makedirs(rootdir+"/locale", exist_ok=True)
    for script in glob.glob(rootdir+"/**/*.lua", recursive=True):
        output = script.replace(".lua", ".en.po").replace(rootdir+"/", rootdir+"/locale/")
        info = {}
        key = None
...
make_scripts_locale("scripts")
make_scripts_locale("mod")
